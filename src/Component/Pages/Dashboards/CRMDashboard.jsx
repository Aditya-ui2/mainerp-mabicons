...
const TeamReportTab = lazy(() => import('./Tabs/Common/TeamReportTab'));
const SuperAdminMISTab = lazy(() => import('./Tabs/Common/SuperAdminMISTab'));
...
const sidebarConfig = [
  {
    items: [
      { id: 'All Employees', title: 'All Employees', icon: FiUsers },
      { id: 'All Clients', title: 'All Clients', icon: FiBriefcase },
      { id: 'Client Onboarding', title: 'Client Onboarding', icon: FiUserPlus },
      { id: 'Client Review', title: 'Client Review', icon: FiCheckSquare },
      { id: 'Client Meeting', title: 'Client Meeting', icon: FiCalendar },
      {
        id: 'Help & Support',
        title: 'Help & Support',
        icon: FiHelpCircle,
      ...
      { id: 'Tasks', title: 'Tasks', icon: FiCheckSquare }
    ]
  }
];
...
const CRMDashboard = () => {
  ...
  const [perfDepartmentFilter, setPerfDepartmentFilter] = useState('All Departments');
  const [perfDateFilter, setPerfDateFilter] = useState('This Month');
  const navigate = useNavigate();
  const [activeTab, setActiveTab] = useState(() => {
    const saved = localStorage.getItem('crm_active_tab');
    return saved === 'HR Policy' ? 'Dashboard' : (saved || 'Dashboard');
  });
  ...
  const handleCreateNote = async (e) => {
    ...
    try {
      const payload = {
        title: noteTitle,
        content: noteContent,
        department: localStorage.getItem('department') || 'HR Operations',
      };
      const res = await createNote(payload);
      if (res.status === 'success' || res.success || res.note || res.data) {
  ...
  const [userInfo, setUserInfo] = useState({ name: 'User', role: 'CRM', avatar: '', picture: '' });

  const refreshUserInfo = () => {
    setUserInfo({
      name: localStorage.getItem('userName') || 'User',
      role: localStorage.getItem('userRole') || localStorage.getItem('userType') || 'CRM',
    ...
    if (rawToken) {
      try {
        ...
        if (base64Url) {
          ...
          setUserInfo({
            name: localStorage.getItem('userName') || decoded.name || decoded.email?.split('@')[0] || 'User',
            role: localStorage.getItem('userRole') || localStorage.getItem('userType') || decoded.role || 'CRM',
            avatar: localStorage.getItem('userPicture') || '',
            picture: localStorage.getItem('userPicture') || '',
          });
        }
  ...
  const fetchData = async (silent = false) => {
    ...
    try {
      ...
      const apiNotes = notesRes.status === 'fulfilled'
        ...
        : [];
      const filteredNotes = Array.isArray(apiNotes) ? apiNotes.filter(n =>
        n.department &&
        n.department.trim().toLowerCase() === dept.trim().toLowerCase()
      ) : [];
      ...
      const leadList = leadsRes.status === 'fulfilled'
        ...
        : [];
      const mappedLeads = Array.isArray(leadList) ? leadList.map(lead => ({
        ...lead,
        personName: lead.personName || lead.contactPerson || 'N/A'
      })) : [];
  ...
  const breadcrumbs = [
    { label: 'Home', path: '/' },
    { label: 'Sales', path: '/crm-dashboard' },
    { label: activeTab }
  ];
        ...
        <div className="space-y-6 pt-0">
          <AnimatePresence mode="wait">
            ...
            {activeTab === 'Dashboard' && (
              ...
              >
                  ...
                  <div className="flex items-center justify-between mb-6">
                    ...
                    <div className="flex items-center gap-6">
                      <div className="flex items-center gap-3">
                        <select
                          value={perfDepartmentFilter}
                          onChange={(e) => setPerfDepartmentFilter(e.target.value)}
                          className="px-3 py-1.5 bg-[#F8FAFF] border border-[#E5E7EB] rounded-xl text-[11px] font-bold text-[#6B6B7E] uppercase focus:outline-none cursor-pointer"
                        >
                        ...
                        <select
                          ...
                          onChange={(e) => setPerfDateFilter(e.target.value)}
                          className="px-3 py-1.5 bg-[#F8FAFF] border border-[#E5E7EB] rounded-xl text-[11px] font-bold text-[#6B6B7E] uppercase focus:outline-none cursor-pointer"
                        >
                      ...
                      <div className="flex gap-4">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-[10px] font-bold text-[#6B6B7E] uppercase">
                            Total Leads
                          </span>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                  ...
                  {/* Recent Leads */}
                  <div className="bg-white rounded-[32px] border border-[#F4F3EF] overflow-hidden min-h-[380px]">

                    <div className="p-6 border-b border-[#F4F3EF] flex items-center gap-4">
                      <div className="w-10 h-10 rounded-2xl bg-[#F8FAFF] flex items-center justify-center text-[#1B4DA0]">
                        <FiUsers size={18} />
                      </div>
                  ...
                  {/* Notes */}
                  <div className="group bg-white rounded-[40px] shadow-sm border border-[#F4F3EF] overflow-hidden min-h-[380px]">
                    ...
                    <div className="h-[280px]">
                      {notes.length === 0 ? (
                        <div className="h-full flex flex-col items-center justify-center text-center">
                          <div className="w-14 h-14 rounded-2xl border border-[#E5E7EB] flex items-center justify-center mb-4">
                            <FiEdit3 size={22} />
                          </div>
                          ...
                          <p className="text-[11px] text-[#9B9BAD] uppercase tracking-widest mt-2">
                            Team directives will appear here
                          </p>
                        </div>
                      ) : (
                        <div className="h-full overflow-y-auto custom-scrollbar space-y-4 pr-1 text-left">
                          {notes.slice(0, 3).map((note, index) => {
                            ...
                            return (
                              <div
                                ...
                                className="p-5 bg-[#FAFAFA] border border-[#F4F3EF] rounded-2xl hover:shadow-md transition-all duration-300 relative cursor-pointer"
                              >
                                <div className="flex justify-between items-start gap-4">
                                  <div className="min-w-0 flex-1">
                                    <h4 className="text-[14px] font-bold text-[#1A1A2E] truncate font-syne mb-1">
                                      {note.title}
                                    </h4>
                                    <p className="text-[#64748B] text-[12px] font-medium leading-relaxed line-clamp-2">
                                      {note.content}
                                    </p>
                                  </div>
                                  <span className="text-[9px] font-bold text-[#9B9BAD] uppercase tracking-[1px] shrink-0">
                                    {dateStr}
                                  </span>
                                </div>
                              </div>
            ...
            {activeTab === 'Client Onboarding' && (
              ...
              >
                <Suspense fallback={<div className="py-20 text-center"><div className="w-8 h-8 border-4 border-[#1B4DA0] border-t-transparent rounded-full animate-spin mx-auto" /></div>}>
                  <CompleteOnboardingTab notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}
            ...
            {activeTab === 'Client Review' && (
              ...
              >
                <Suspense fallback={<div className="py-20 text-center"><div className="w-8 h-8 border-4 border-[#1B4DA0] border-t-transparent rounded-full animate-spin mx-auto" /></div>}>
                  <ClientReviewTab notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}

            {activeTab === 'Client Meeting' && (
              <motion.div
                key="meeting"
                initial={{ opacity: 0, y: 10 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <Suspense fallback={<div className="py-20 text-center"><div className="w-8 h-8 border-4 border-[#1B4DA0] border-t-transparent rounded-full animate-spin mx-auto" /></div>}>
                  <MeetingWithClientTab notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}
            ...
            {activeTab === 'All Employees' && (
              <motion.div
                ...
                transition={{ duration: 0.2 }}
              >
                <TeamTabs isDarkMode={false} notificationBell={renderNotificationBell()} />
              </motion.div>
            )}
            ...
            {activeTab === 'HR Policy' && (
              ...
              >
                <Suspense fallback={<div className="py-20 text-center"><div className="w-8 h-8 border-4 border-[#1B4DA0] border-t-transparent rounded-full animate-spin mx-auto" /></div>}>
                  <PolicyTab isDarkMode={false} notificationBell={renderNotificationBell()} isReadOnly={true} />
                </Suspense>
              </motion.div>
            )}
            ...
            {activeTab === 'Tasks' && (
              ...
              >
                <Suspense fallback={<div className="py-20 text-center"><div className="w-8 h-8 border-4 border-[#1B4DA0] border-t-transparent rounded-full animate-spin mx-auto" /></div>}>
                  <TaskAssignmentTab department="CRM" notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}
              ...
              >
                <Suspense fallback={
                  <div className="flex items-center justify-center h-[50vh]">
                    <div className="w-12 h-12 border-4 border-[#F4F3EF] border-t-[#0D47A1] rounded-full animate-spin"></div>
                  </div>
                }>
                  <TeamReportTab notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}
            ...
            {activeTab === 'MIS Reports' && (
              ...
              >
                <Suspense fallback={
                  ...
                  <SuperAdminMISTab notificationBell={renderNotificationBell()} />
                </Suspense>
              </motion.div>
            )}
          ...
          {!['Dashboard', 'Client Pipeline', 'Client Onboarding', 'Client Review', 'Client Meeting', 'All Clients', 'All Employees', 'Help & Support', 'Internal', 'External', 'HR Policy', 'Notes', 'My Profile', 'Team Report', 'Tasks'].includes(activeTab) && (
          ...
          {/* Create Note Modal Portal */}
          {createPortal(
            <AnimatePresence>
              {showCreateNoteModal && (
                <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4 overflow-x-hidden overflow-y-auto pointer-events-none text-left">
                  {/* Backdrop */}
                  <motion.div
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                    exit={{ opacity: 0 }}
                    className="fixed inset-0 bg-black/40 backdrop-blur-md z-0 pointer-events-auto"
                  ...
                  <motion.div
                    ...
                    transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                    className="relative w-full max-w-xl bg-white rounded-3xl overflow-hidden shadow-2xl z-10 flex flex-col my-8 pointer-events-auto border border-[#F4F3EF]"
                  >
                    {/* Header */}
                    <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-gradient-to-r from-white to-[#F8FAFF]">
                      ...
                      <button onClick={() => setShowCreateNoteModal(false)}
                    ...
                    {/* Body */}
                    <form onSubmit={handleCreateNote} className="p-10 max-h-[75vh] overflow-y-auto custom-scrollbar space-y-8">
          ...
          {selectedKpi && (
            <div className="fixed inset-0 z-[9999] flex items-center justify-center p-4">
              ...
              <motion.div
                ...
                exit={{ opacity: 0, scale: 0.9, y: 20 }}
                transition={{ type: 'spring', damping: 25, stiffness: 300 }}
                className="relative bg-white w-full max-w-sm rounded-[32px] p-8 shadow-2xl border border-white/20 text-center"
              >
                <button
                  onClick={() => setSelectedKpi(null)}
                ...
                <div className="text-5xl font-black text-[#1B4DA0] tracking-tight mb-6">
                  {selectedKpi.value}
                </div>

                <p className="text-sm font-medium text-gray-500 leading-relaxed bg-gray-50 rounded-2xl p-4 border border-gray-100">
                  {selectedKpi.desc}
                </p>

                <button
                  onClick={() => setSelectedKpi(null)}
                  className="mt-8 w-full py-3.5 bg-gray-900 hover:bg-black text-white rounded-xl font-bold text-sm tracking-wide transition-colors"
                >
                  Close
                </button>
              </motion.div>

...
const sidebarConfig = [
  {
    ...
    items: [
      ...
      { id: 'Reports', title: 'Reports', icon: FiBarChart2 },
...
const BDDashboardNew = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('bd_active_tab') || 'Dashboard');
  const [leads, setLeads] = useState([]);
  const [loading, setLoading] = useState(true);
  const [userInfo, setUserInfo] = useState({ name: 'BD Executive', role: 'Business Development' });

  useEffect(() => {
    localStorage.setItem('bd_active_tab', activeTab);
  }, [activeTab]);
  ...
  const refreshUserInfo = () => {
    ...
    let fallbackName = 'BD Executive';
    let fallbackRole = 'Business Development';
    try {
      const token = localStorage.getItem('token');
      if (token) {
        const decoded = JSON.parse(atob(token.split('.')[1]));
        fallbackName = decoded.name || decoded.email?.split('@')[0] || 'BD Executive';
        fallbackRole = decoded.role || 'Business Development';
      }
    } catch (e) {}
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'Announcements':
        return <AnnouncementsTab department="BD" isHead={false} />;
      default:
        return (
          <div className="space-y-6">
            {/* Welcome Banner */}
            ...
            {/* Recent Leads and Announcements Grid */}
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-1">
                <AnnouncementsWidget department={localStorage.getItem('department') || 'All'} />
              </div>
            </div>
          </div>
        );

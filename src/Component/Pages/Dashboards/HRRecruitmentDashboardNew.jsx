...
import { getAllNotifications, markNotificationRead, getRecruitmentStats, getAllRecruitmentPositions, getAllInterviews, getMyProfile } from '../service/api';
...
const sidebarConfig = [
  ...
  },
  {
    ...
    items: [
      ...
      { id: 6, title: 'Shortlisted Candidates', icon: FiCheckCircle },
    ]
  },
  {
    heading: 'ANALYTICS',
...
const HRRecruitmentDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('recruitment_active_tab') || 'Dashboard');
  const [loading, setLoading] = useState(false);
  const [userInfo, setUserInfo] = useState({ name: 'HR Recruitment', role: 'HR Recruitment Head' });
  const [notifications, setNotifications] = useState([]);

  useEffect(() => {
    localStorage.setItem('recruitment_active_tab', activeTab);
  }, [activeTab]);
  ...
  const refreshUserInfo = () => {
    ...
    setUserInfo({
      name: localName || 'HR Recruitment',
      role: localRole || 'HR Recruitment Head',
      avatar: localPicture || ''
    });
  };
  ...
  useEffect(() => {
    ...
    return () => {
      window.removeEventListener('profileUpdate', refreshUserInfo);
        ...
        {(() => {
          switch (activeTab) {
            ...
            case 'Shortlisted Candidates':
              return <ShortlistedCandidatesTab />;

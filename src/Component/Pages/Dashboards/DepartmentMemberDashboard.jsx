...
const DepartmentMemberDashboard = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('dept_member_active_tab') || 'Dashboard');
  const [userInfo, setUserInfo] = useState({ name: 'Team Member', role: 'Team Member' });
  const [department, setDepartment] = useState('');

  useEffect(() => {
    localStorage.setItem('dept_member_active_tab', activeTab);
  }, [activeTab]);
  ...
  const refreshUserInfo = () => {
    ...
    const cleanName = (localName || 'Team Member').replace(/\s*\(.*?\)\s*$/, '').trim();
    setUserInfo({
      name: cleanName,
      role: localRole || 'Team Member',
      avatar: localPicture || ''
    });

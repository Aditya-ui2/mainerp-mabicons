...
const getSidebarConfig = (name = '') => {
  ...
  const items = [
    ...
    { id: 16, title: 'Shortlisted Candidates', icon: FiCheckCircle },
...
const ClientsTab = ({ distribution, handovers, onViewClient, onSync, notificationBell }) => {
        ...
        >
          ...
          <div className="p-8 border-b border-[#F4F3EF] bg-gradient-to-br from-blue-50/50 to-white flex items-center justify-between relative overflow-hidden">
            ...
            <div className="flex items-center gap-6 relative z-10">
              ...
              <div>
                <h3 className="text-2xl font-medium text-[#1A1A2E] leading-tight" style={{ fontFamily: "'Syne', sans-serif" }}>
                  {client.name}
                </h3>
                <p className="text-[10px] font-medium text-blue-600 uppercase tracking-[3px] mt-1">Client Profile Protocol</p>
              </div>
            </div>
            <button
                onClick={onClose}
          ...
          <div className="flex flex-col items-center text-center">
            ...
            <div className="space-y-1.5">
              <h4 className="text-xl font-semibold text-[#1A1A2E] tracking-tight" style={{ fontFamily: "'Syne', sans-serif" }}>{client.name}</h4>
              <p className="text-[12px] font-semibold text-[#1B4DA0] tracking-tight uppercase tracking-[3px]">{client.industry || 'Enterprise'} Sector</p>
            </div>
          </div>
...
const KAMMemberDashboard = () => {
  ...
  const refreshUserInfo = () => {
    ...
    setUserInfo(prev => ({
      ...prev,
      name: localName || prev.name || 'KAM',
      role: localRole || prev.role || 'Key Account Manager - Recruitment',
  ...
  useEffect(() => {
    ...
    if (token) {
      try {
        ...
        setUserInfo(prev => ({
          ...prev,
          name: userName || decoded.name || 'KAM',
          role: localStorage.getItem('userRole') || localStorage.getItem('userType') || decoded.role || 'Key Account Manager - Recruitment',
          id: userId,
        ...
        interval = setInterval(() => fetchNotifications(userId), 60000);
      } catch (e) {
        setUserInfo(prev => ({
          ...prev,
          name: userName || 'KAM',
          role: localStorage.getItem('userRole') || localStorage.getItem('userType') || 'Key Account Manager - Recruitment',
          id: null,
          email: userEmail,
  ...
  const fetchNotifications = async (userId) => {
    try {
      ...
      const notifs = (res.data || [])
        .filter(n => !n.message.startsWith('📢 Announcement:') && !n.message.includes('Announcement:'))
        .map((n) => ({
          id: n.id,
          text: n.message,
  ...
  const fetchClients = async () => {
    try {
      ...
      // Filter by assignment from localStorage or fallback to backend-filtered clients list
      ...
      const assignedToMe = allClients.filter(client => {
        // If client.assignKAM directly matches our name, it's ours!
        if (client.assignKAM) {
          const clientKAM = client.assignKAM.toLowerCase().trim();
          if (clientKAM.includes(currentUserName) || currentUserName.includes(clientKAM)) {
            return true;
          }
        }
      ...
      // Use assignedToMe filtered list if not empty; otherwise, fallback to allClients returned from backend
      // since the backend has already filtered the database entries based on the logged-in user's name
      const finalClients = assignedToMe.length > 0 ? assignedToMe : allClients;

      setClientJobDistribution(finalClients);
    } catch (error) {
    ...
    return (
      <Suspense fallback={<TabLoader />}>
        {(() => {
          switch (activeTab) {
            case 'My Tasks':
              return <TaskAssignmentTab department="HR Recruitment" notificationBell={renderNotificationBell()} forceMyTasksOnly={true} />;
            case 'Daily Report':
              return <DailyReportTab notificationBell={renderNotificationBell()} />;
            case 'Job Openings':
              return <JobOpeningsTab notificationBell={renderNotificationBell()} />;
            case 'Candidate Pipeline':
              return (
                <CandidatePipelineTab notificationBell={renderNotificationBell()} />
              );
            ...
            case 'Screening':
              return <ScreeningTab notificationBell={renderNotificationBell()} />;
            case 'Offer Management':
              return <OfferManagementTab notificationBell={renderNotificationBell()} />;
            case 'Shortlisted Candidates':
              return <ShortlistedCandidatesTab isDarkMode={false} notificationBell={renderNotificationBell()} />;
            case 'Resume Bank':
              return <ResumeBankTab notificationBell={renderNotificationBell()} />;
            case 'Activity Feed':
            ...
            case 'My Team':
              return <TeamMembersTab notificationBell={renderNotificationBell()} />;
              ...
              return (
                <div className="space-y-8 bg-[#FDFDFD] -m-6 p-6 min-h-screen">
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                    ...
                    {/* Upcoming Joinings */}
                    <div className="lg:col-span-2 bg-white rounded-[32px] shadow-sm overflow-hidden flex flex-col border border-[#F4F3EF]">

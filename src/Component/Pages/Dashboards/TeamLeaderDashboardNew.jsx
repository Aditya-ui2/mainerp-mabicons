...
const TeamLeaderDashboardNew = () => {
  ...
  const [activeTab, setActiveTab] = useState(() => localStorage.getItem('tl_active_tab') || 'Dashboard');
  const [loading, setLoading] = useState(false);
  const [userInfo, setUserInfo] = useState({ name: 'Team Leader', role: 'Team Leader' });

  useEffect(() => {
    localStorage.setItem('tl_active_tab', activeTab);
  }, [activeTab]);
  ...
  const refreshUserInfo = () => {
    ...
    let fallbackName = 'Team Leader';
    let fallbackRole = 'Team Leader';
    try {
      const token = localStorage.getItem('token');
      if (token) {
        const decoded = JSON.parse(atob(token.split('.')[1]));
        fallbackName = decoded.name || decoded.email?.split('@')[0] || 'Team Leader';
        fallbackRole = decoded.role || 'Team Leader';
      }
    } catch (e) {}
  ...
  const renderContent = () => {
    switch (activeTab) {
      ...
      case 'Announcements':
        return <AnnouncementsTab department={localStorage.getItem('department') || 'All'} isHead={false} />;
      case 'Onboarding':
        return (

...
import { FiPlus, FiCheck, FiX, FiChevronDown, FiChevronRight, FiInfo, FiFileText, FiUpload, FiMail, FiUsers, FiBriefcase, FiTarget, FiZap, FiPhone, FiSearch, FiRefreshCw, FiCheckCircle, FiEdit2, FiCamera, FiEye, FiTrash, FiDownload } from 'react-icons/fi';
...
const CRMTeamTab = ({ isDarkMode, notificationBell, department }) => {
  ...
  const downloadExistingMemberDoc = (docId) => {
    ...
    try {
      ...
      link.href = url;
      link.download = docData.name || `${docId}_document`;
    ...
    <div className="space-y-8 animate-in fade-in duration-500" style={{ fontFamily: "'Calibri', sans-serif" }}>
      ...
      <div className="flex items-center justify-between mb-8 flex-wrap gap-4 text-left">
        ...
        <div className="flex items-center gap-2">
          {notificationBell}
          {/* Add Button */}
          {(!userRole?.toLowerCase()?.includes('tech') && !userRole?.toLowerCase()?.includes('client') && !userRole?.toLowerCase()?.includes('customer')) && (
            <button
              onClick={() => {
                setModalStep(1);
                setIsAddMemberModalOpen(true);
        ...
        <div className="overflow-x-auto min-h-[300px]">
          ...
          <div className="grid grid-cols-[40px_220px_150px_180px_120px_120px_130px_340px_60px] gap-8 px-8 py-4 border-b border-[#F4F3EF] bg-transparent min-w-[1500px]">
            ...
            {[
              ...
              <div key={i} className={`text-[11px] font-bold text-[#94a3b8] uppercase tracking-widest text-left flex items-start ${h === 'Actions' ? 'justify-end text-right w-full pr-4' : ''}`}>
                {h}
              </div>
            ))}
          </div>
              ...
              >
                ...
                <div className="flex items-center" onClick={(e) => e.stopPropagation()}>
                  <button
                    onClick={(e) => {
                      e.stopPropagation();
                      setSelectedDocsMember({ ...member, documents: member.documents || {} });
          ...
          {selectedIds.length > 0 && (
            ...
            >
              ...
              <div className="flex items-center gap-6 flex-1 justify-center text-white">
                ...
                <button
                  onClick={() => {
                    const selectedMemberData = flattenedMembers.find(
                      m => m._id === selectedIds[0] || m.id === selectedIds[0]
                    );

                    if (!selectedMemberData) return;

                    setMemberToDelete(selectedMemberData);
                    setIsDeleteModalOpen(true);
                  }}
                  className="flex items-center gap-2 group px-4 py-2 rounded-2xl transition-all hover:bg-red-500/20"
                >
                  <FiTrash size={16} className="text-red-400" />
                          ...
                          {selectedDocsMember?.documents?.[doc.id] ? (
                            <>
                              ...
                              <button
                                ...
                                className="w-9 h-9 rounded-lg bg-white text-[#10B981] hover:bg-emerald-50 transition-all flex items-center justify-center shadow-sm border border-emerald-100"
            ...
            <div className="space-y-8">
              ...
              <div className="bg-[#FAFAF8] rounded-[32px] p-8 space-y-6 border border-[#F4F3EF]">
                <div className="space-y-4">
                  ...
                  {[
                    { label: 'Basic Salary', amount: selectedMember?.basicSalary !== undefined && selectedMember.basicSalary !== null ? `₹ ${Number(selectedMember.basicSalary).toLocaleString('en-IN')}` : '₹ 0' },

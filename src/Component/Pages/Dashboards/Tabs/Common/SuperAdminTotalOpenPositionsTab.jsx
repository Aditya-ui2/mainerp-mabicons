...
const formatSalaryDisplay = (value) => {
  const salary = String(value || '').replace(/\$/g, 'â‚¹').trim();
  if (!salary) return 'Not specified';

  // If rupee/INR already present, keep as-is.
  if (/â‚¹|\brs\b|\binr\b/i.test(salary)) return salary;
...
const AssignTaskModal = ({ isDarkMode, job, onClose, onAssign, teamMembers = [] }) => {
  ...
  const priorities = [
    { value: 'High', color: '#ef4444', bg: isDarkMode ? 'bg-red-900/30 border-red-700/50 text-red-400' : 'bg-red-50 border-red-200 text-red-600', icon: 'ðŸ”´' },
    { value: 'Medium', color: '#f59e0b', bg: isDarkMode ? 'bg-[#1E88E5]/30 border-[#1E88E5]/50 text-[#3FA9F5]' : 'bg-[#1E88E5]/10 border-[#1E88E5]/30 text-[#1E88E5]', icon: 'ðŸŸ¡' },
    { value: 'Low', color: '#10b981', bg: isDarkMode ? 'bg-emerald-900/30 border-emerald-700/50 text-emerald-400' : 'bg-emerald-50 border-emerald-200 text-emerald-600', icon: 'ðŸŸ¢' },
  ];
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="flex flex-wrap items-center gap-2 mt-0.5">
                  ...
                  <span className="text-xs text-[#9B9BAD]">â€¢</span>
    ...
    <div className="flex flex-col h-full bg-white relative animate-in fade-in slide-in-from-right duration-500">
      ...
      <div className="flex-1 overflow-y-auto px-10 py-8 space-y-10 custom-scrollbar">
        ...
        <div className="flex flex-col items-center text-center">
          ...
          <div className="space-y-1.5 w-full">
            {isEditing ? (
              ...
              <h4
                className="text-2xl font-bold text-[#1A1A2E] tracking-tight transition-colors flex items-center justify-center gap-3"
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="bg-[#FAFAF8] p-4 rounded-3xl border border-[#F4F3EF] text-center transition-all">
        ...
        <div className="bg-[#FAFAF8] rounded-[32px] border border-[#F4F3EF] p-10 space-y-8 shadow-sm">
          ...
          <div className="flex items-center justify-between pt-8 border-t border-[#F4F3EF] group">
...
const SuperAdminTotalOpenPositionsTab = ({ isDarkMode, notificationBell }) => {
  ...
  // â”€â”€ Fetch clients from backend â”€â”€
  ...
  // â”€â”€ Fetch team members from backend â”€â”€
  ...
  // â”€â”€ Fetch positions from backend (always background) â”€â”€
    ...
    <>
      ...
      <div className="space-y-8" style={{ fontFamily: "'Calibri', sans-serif" }}>
        ...
        <div className="bg-white rounded-[32px] border border-[#F4F3EF] overflow-hidden shadow-sm">
          <div className="grid grid-cols-[2fr_1.5fr_120px_130px_100px_40px] gap-4 px-8 py-4 border-b border-[#F4F3EF] bg-transparent">
            {["Position", "Client", "Status", "Posted", "Applicants", ""].filter(Boolean).map((h, i) => {
          ...
          {filteredJobs.length === 0 ? (
            ...
            filteredJobs.map((job) => (
              <div
                ...
                className="grid grid-cols-[2fr_1.5fr_120px_130px_100px_40px] gap-4 items-center px-8 py-3 border-b border-[#F4F3EF] last:border-0 hover:bg-[#F8FAFF] cursor-pointer transition-all group relative job-selection-row"
              >
      ...
      {/* Selection Action Bar Removed for Super Admin */}
...
/* â”€â”€ Bulk Job Preview Modal â”€â”€ */
...
export default SuperAdminTotalOpenPositionsTab;

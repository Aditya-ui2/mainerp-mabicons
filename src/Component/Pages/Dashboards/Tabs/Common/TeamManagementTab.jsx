...
const TeamManagementTab = ({ department = 'HR Operations' }) => {
  ...
  return (
    <div className="space-y-8 animate-in fade-in duration-500">
          ...
          {canAddMember && (
            <button
              onClick={() => {
                setEditingMember(null);
                setFormData({
                  name: '', email: '', phone: '', employeeId: '', department: department,
                  joiningDate: '', profilePhoto: null, profilePhotoPreview: null, status: 'Active'
                });
                setShowModal(true);
              }}
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="rounded-[24px] p-2 border flex items-center gap-3 flex-wrap mb-6 bg-white border-[#F4F3EF] shadow-sm">
        <div className="relative flex-1 group min-w-[200px]">
          ...
          <input
            ...
            className="w-full border-none rounded-2xl py-3 pl-14 pr-5 text-sm font-medium outline-none transition-all placeholder:text-[#9B9BAD] bg-[#F4F3EF] text-[#1A1A2E] focus:ring-2 focus:ring-[#F4F3EF]"
          />
          {searchTerm && (
      ...
      {/* Team Directory Modern Table */}
      <div className="rounded-[32px] border overflow-hidden transition-all bg-white border-[#F4F3EF] shadow-sm">
        <div className="hidden md:grid grid-cols-[40px_1.5fr_1fr_1fr_150px_40px] gap-4 px-8 py-4 border-b border-[#F4F3EF] bg-transparent">
          {["", "Member", "Department", "Email Address", "Connect", ""].map((h, i) => (
            ...
            {filteredMembers.length === 0 ? (
              ...
              filteredMembers.map((member, index) => (
                <motion.div
                  ...
                  className="grid grid-cols-[1fr] md:grid-cols-[40px_1.5fr_1fr_1fr_150px_40px] gap-4 items-center px-8 py-3 border-b border-[#F4F3EF] last:border-0 cursor-pointer transition-all group relative hover:bg-[#F8FAFF]"
                >
                  <div className="px-1 text-left" onClick={(e) => e.stopPropagation()}>
                    <input
                      type="checkbox"
                  ...
                  <div className="flex items-center gap-4 min-w-0 py-1">
                    <div className="w-[42px] h-[42px] rounded-[14px] flex items-center justify-center font-bold shadow-sm transition-transform group-hover:scale-110 bg-[#F0F7FF] text-[#1B4DA0] text-[13px] overflow-hidden shrink-0">
                    ...
                    <div className="flex flex-col min-w-0 text-left">
                      <p className="text-[14px] font-bold truncate transition-colors text-[#0f172a] group-hover:text-[#0D47A1]">
                        {member.name}
                      </p>
                  ...
                  <div className="text-left hidden md:flex flex-col justify-center">
                    <p className="text-[13px] font-bold text-[#6B6B7E] truncate">{member.department}</p>
                  ...
                  <div className="text-left hidden md:block">
                     <p className="text-[13px] font-bold text-[#6B6B7E] truncate">{member.email}</p>
                  </div>

                  <div className="text-left hidden md:flex items-center gap-2">
                    <button onClick={(e) => { e.stopPropagation(); window.location.href = `mailto:${member.email}`; }} className="p-2 bg-[#F4F3EF] text-[#9B9BAD] hover:text-[#1B4DA0] rounded-xl transition-all shadow-sm">
                      <FiMail size={16} />
                    </button>
                    <button onClick={(e) => { e.stopPropagation(); window.location.href = `tel:${member.phone}`; }} className="p-2 bg-[#F4F3EF] text-[#9B9BAD] hover:text-[#1B4DA0] rounded-xl transition-all shadow-sm">
                      <FiPhone size={16} />
                    </button>
                  </div>

                  <div className="flex justify-end pr-2">
                    <FiChevronRight size={20} className="transition-all text-[#C5C5D2] group-hover:text-[#0D47A1]" />
                  </div>
                </motion.div>
              ))
            )}
                  ...
                  <div className="bg-[#FAFAF9] rounded-[40px] p-8 border border-[#F4F3EF] flex flex-col items-center text-center">
                    <div className="w-24 h-24 rounded-[32px] bg-gradient-to-br from-white to-[#F4F3EF] flex items-center justify-center text-[#1B4DA0] text-3xl font-black shadow-2xl border border-[#F4F3EF] group-hover:scale-105 transition-transform overflow-hidden relative">
                      ...
                      {editableMember?.profilePhoto && (String(editableMember.profilePhoto).includes('data:image') || String(editableMember.profilePhoto).includes('http')) && (
                        <img 
                          src={editableMember.profilePhoto} 
                          alt="" 

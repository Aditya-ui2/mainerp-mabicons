...
const HelpSupportTab = ({ userRole = '', userName = 'Current User', notificationBell }) => {
      ...
      const mockData = [
        {
          ...
          description: 'Unable to login to the candidate portal sometimes. Getting a 502 bad gateway error on chrome browser when submitting resume.',
          priority: 'High',
          status: 'Pending',
          raisedBy: 'John Doe',
          raisedByRole: 'HR Recruiter',
        ...
        {
          id: 'PRB-002',
          title: 'Need extra job slots',
          description: 'Client needs 50 more openings added to the active listing. Currently reached maximum allowed slots on Recruitment portal.',
          priority: 'Medium',
          status: 'Resolved',
          raisedBy: 'Sachin (Recruitment Head)',
          raisedByRole: 'Recruitment Head',
          raisedAt: new Date(Date.now() - 172800000).toISOString(),
          resolvedBy: 'Admin User',
          resolvedAt: new Date(Date.now() - 86400000).toISOString()
        }
      ];
      setProblems(mockData);
      localStorage.setItem('erp_help_support_problems', JSON.stringify(mockData));
    }
  }, []);

  const saveProblems = (newProblems) => {
    setProblems(newProblems);
    localStorage.setItem('erp_help_support_problems', JSON.stringify(newProblems));
  };
  ...
  const handleSubmitProblem = (e) => {
    ...
    const newProblem = {
      ...
      status: 'Pending',
      raisedBy: userName,
      raisedByRole: userRole || 'Employee',
  ...
  const handleBulkResolve = () => {
    const updated = problems.map(p => {
      if (selectedIds.includes(p.id) && p.status === 'Pending') {
        return {
          ...p,
          status: 'Resolved',
          resolvedBy: userName,
  ...
  const filteredProblems = problems.filter(p => {
    ...
    if (activeTab === 'My Problems') {
      ...
      if (isSuperAdmin || isCRM) {
        ...
        return p.raisedByRole.toLowerCase().includes('head') || p.raisedBy !== userName;
      } else if (isRecruitmentHead) {
        // Recruitment Head sees problems raised by their team (Recruiters, KAMs)
        return !p.raisedByRole.toLowerCase().includes('head') && !p.raisedByRole.toLowerCase().includes('admin') && p.raisedBy !== userName;
      }
      return false;
    }
  });
      ...
      <div className="flex items-center justify-between gap-4 mb-2">
        ...
        <div className="flex items-center gap-3">
          ...
          <button
            ...
            className="flex items-center gap-2 px-6 py-3 bg-[#1B4DA0] text-white rounded-2xl text-[11px] font-bold uppercase tracking-widest hover:bg-[#153e82] transition-all active:scale-95 shadow-lg shadow-blue-500/20"
          >
            <FiPlus size={14} />
            Raise a Problem
          </button>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="border-b border-[#F4F3EF]">
                ...
                <th className="px-8 py-4 text-[11px] font-black text-[#9B9BAD] uppercase tracking-[3px] w-36">Status</th>
            ...
            <>
              ...
              >
                ...
                <div className="p-8 border-t border-[#F4F3EF] bg-[#FBFBFF] flex gap-4">
                  ...
                  {selectedProblemForDetail.raisedBy === userName && (
                    <button

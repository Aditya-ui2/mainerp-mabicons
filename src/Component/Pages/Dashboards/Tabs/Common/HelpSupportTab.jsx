...
const HelpSupportTab = ({ userRole = '', userName = 'Current User', notificationBell }) => {
  ...
  const actualRole = userRole || (typeof window !== 'undefined' ? (localStorage.getItem('userRole') || localStorage.getItem('userType')) : '') || 'Employee';
  const actualUser = (userName && userName !== 'Current User') ? userName : (typeof window !== 'undefined' ? (localStorage.getItem('userName') || localStorage.getItem('userEmail')) : 'Current User');
  ...
  const [loading, setLoading] = useState(true);
   ...
   const fetchProblems = async () => {
    try {
      ...
      console.log("GET Problems Response:", res);
  ...
  const filteredProblems = problems.filter(p => {
    ...
    if (activeTab === 'My Problems') {
      ...
      if (isSuperAdmin || isCRM) {
        ...
        return !p.raisedByRole.toLowerCase().includes('head') && !p.raisedByRole.toLowerCase().includes('admin') && p.raisedBy !== actualUser;
      }
      return false;
    }
  });
              ...
              {loading ? (
                ...
                filteredProblems.map((problem) => (
                  ...
                  >
                    ...
                    <td className="px-8 py-8 whitespace-nowrap">
                      <div className="flex flex-col text-left">
                        <span className="text-xs font-bold text-[#4B4B5E]">
                ...
                <div className="flex-1 overflow-y-auto p-8 custom-scrollbar space-y-8 text-left">
                  ...
                  <div className="space-y-6">
                    <div className="flex items-center justify-between px-2">
                      <span className="text-[11px] font-black text-[#9B9BAD] uppercase tracking-[3px]">Context</span>
                    ...
                    <div className="grid grid-cols-2 gap-4 px-2">
                      ...
                      <div className="space-y-2">
                        ...
                        <div className="flex items-center gap-3 p-4 bg-[#FAFAF9] rounded-2xl border border-[#F4F3EF]">
                          ...
                          <div className="flex flex-col text-left">
                            <span className="text-xs font-bold text-[#1A1A2E]">{new Date(selectedProblemForDetail.createdAt).toLocaleDateString()}</span>
                            <span className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider mt-0.5">
                              {new Date(selectedProblemForDetail.createdAt).toLocaleTimeString([], {
                                    hour: '2-digit',
                                    minute: '2-digit'
                                })}
                            </span>

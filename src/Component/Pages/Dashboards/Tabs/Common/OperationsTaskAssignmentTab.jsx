...
const TaskDetailView = ({ task, onBack, onUpdateTask, showToast, teamMembers = [] }) => {
  ...
  const currentUserRole = useMemo(() => {
    ...
    if (['superadmin', 'super_admin', 'super admin'].includes(normalizedType)) return 'superadmin';
    if (['superadmin', 'super_admin', 'super admin'].includes(normalizedRole)) return 'superadmin';
  ...
  const canEditThisTask = !isAssignedByAshish || isCurrentUserSuperAdmin;
    ...
    try {
      ...
      if (editingTask) {
        ...
        if (typeof taskId === 'string' && taskId.startsWith('mock-')) {
          setTasks(prev => prev.map(t => (t.id === taskId || t._id === taskId) ? { ...t, ...taskData } : t));
            ...
            <tbody className="divide-y divide-gray-50">
              {filteredTasks.length === 0 ? (
                ...
                filteredTasks.map(task => {
                  ...
                  const canDeleteTask = !isTaskAssignedByAshish || currentUserRole === 'superadmin';

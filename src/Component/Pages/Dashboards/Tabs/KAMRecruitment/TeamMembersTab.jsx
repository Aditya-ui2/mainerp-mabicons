...
/* ══════════════════════════════════════════════════════ */
const TeamMembersTab = ({ isDarkMode, userRole = 'KAM', notificationBell }) => {
  ...
  const fetchMembers = async (showSkeleton = false) => {
  ...
  useEffect(() => {
    ...
    if (token && !userId) {
      try {
        ...
        setCurrentUserName(localStorage.getItem('userName') || decoded.name || '');
  ...
  return (
    <div className="min-h-screen" style={{ fontFamily: "'Calibri', sans-serif" }}>
      ...
      <div className="bg-white rounded-[32px] border border-[#F4F3EF] overflow-hidden shadow-sm">
        {/* Header Row */}
        <div className="grid grid-cols-[2fr_1.5fr_2fr_120px_40px] gap-4 px-8 py-4 border-b border-[#F4F3EF] bg-transparent">
        ...
        {filteredMembers.length > 0 ? filteredMembers.map((m) => {
          ...
          return (
            <div 
              ...
              className={`grid grid-cols-[2fr_1.5fr_2fr_120px_40px] gap-4 items-center px-8 py-3 border-b border-[#F4F3EF] last:border-0 hover:bg-[#F8FAFF] cursor-pointer transition-all group ${isSelected ? 'bg-blue-50/30' : ''}`}
            >
              {/* Member */}
          ...
          {showMemberDetails && selectedMember && (
            <>
              ...
              <motion.div
                ...
                className="fixed inset-0 bg-[#1A1A2E]/40 backdrop-blur-md z-[9999]"
              />
              ...
              <motion.div
                ...
                className="fixed inset-y-0 right-0 w-full max-w-[698px] bg-white shadow-2xl z-[10000] border-l border-[#F4F3EF] flex flex-col overflow-hidden"
              >
                {/* Header */}
            ...
            >
              <div className={`px-8 py-6 border-b flex items-center justify-between ${isDarkMode ? 'border-slate-700' : 'border-[#F4F3EF]'}`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className={`text-[10px] font-bold uppercase tracking-[3px] mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-[#9B9BAD]'}`}>
                      to {selectedMember.id === 'bulk' ? `${selectedRowIds.length} Members` : selectedMember.name}
                    </p>
                  </div>
                </div>
              ...
              <div className={`p-8 border-t bg-[#FAFAF9] flex items-center justify-end gap-3 ${isDarkMode ? 'bg-slate-800 border-slate-700' : ''}`}>
                ...
                <button 
                   onClick={async () => {
                        try {
                            if (selectedMember.id === 'bulk') {
                                const targetMembers = members.filter(m => selectedRowIds.includes(m.id));
                                let successCount = 0;
                                ...
                                if (successCount > 0) {
                                    ...
                                    setAssignDueDate('');
                                    setSelectedRowIds([]);
                                    setToast(`Task assigned to ${successCount} members`);
                                }
                            } else {
                                const res = await createDepartmentTask({
                                    title: assignTitle,
        ...
        <AnimatePresence>
          {selectedRowIds.length > 0 && (
            <motion.div
              ...
              className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[1000] flex items-center gap-6 px-10 py-5 bg-[#1A1A2E] rounded-[32px] shadow-2xl shadow-slate-900/40 min-w-[520px] border border-white/5 text-white"
            >
              {/* Count badge */}
              <div className="flex items-center gap-4 pr-8 border-r border-white/10 shrink-0">
                ...
                <div className="text-left flex flex-col justify-center">
                  <p className="text-[14px] font-black text-white tracking-tight whitespace-nowrap">Member{selectedRowIds.length > 1 ? 's' : ''} Selected</p>
                  <button
              ...
              <div className="flex items-center gap-6 flex-1 justify-center text-white">
                ...
                <button
                  onClick={() => {
                    setSelectedMember({
                      id: 'bulk',
                      name: members.filter(m => selectedRowIds.includes(m.id)).map(m => m.name).join(', ')
                    });
                    setShowAssignModal(true);
                  }}
                  className="flex items-center gap-2 group px-4 py-2 rounded-2xl transition-all hover:bg-white/5 active:scale-95"
                >
                  <FiSend size={16} className="text-emerald-400 group-hover:text-white" />
                  <span className="text-[11px] font-bold uppercase tracking-widest">Assign Work</span>
                </button>
              </div>

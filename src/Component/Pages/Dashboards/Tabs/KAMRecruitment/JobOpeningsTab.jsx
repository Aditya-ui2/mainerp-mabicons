  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
      ...
      <motion.div
        ...
        style={{ boxShadow: '0 25px 50px -12px rgba(0,0,0,0.25), 0 0 0 1px rgba(31,136,229,0.1)' }}
      >
        {/* ── Compact Header ── */}
              ...
              {teamMembers.map(m => (
                <motion.button key={m.id} whileHover={{ x: 2 }} whileTap={{ scale: 0.99 }}
                  ...
                  <div className={`w-5 h-5 rounded-full border-2 flex items-center justify-center transition-all ${
                    assignee === m.id ? 'border-[#1E88E5] bg-[#1E88E5]' : isDarkMode ? 'border-slate-600' : 'border-slate-300'
                  }`}>
                  ...
                  <div className={`flex flex-col sm:flex-row sm:items-center justify-between p-3 rounded-xl mb-4 gap-2 ${isDarkMode ? 'bg-slate-700/50' : 'bg-[#1E88E5]/10'}`}>
                    <label className="flex items-center gap-3 cursor-pointer select-none" onClick={toggleSelectAll}>
                      <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center transition-all ${
                        selectedResumes.size === matchedResumes.length ? 'bg-[#1E88E5] border-[#1E88E5]' : isDarkMode ? 'border-slate-500' : 'border-slate-300'
                      }`}>
                      ...
                      return (
                        <motion.div
                          ...
                          className={`flex items-start gap-3 p-3 rounded-xl cursor-pointer transition-all border-2 ${
                            isSelected ? (isDarkMode ? 'border-[#1E88E5] bg-[#1E88E5]/10' : 'border-[#1E88E5] bg-[#1E88E5]/10') : (isDarkMode ? 'border-slate-700 bg-slate-700/30 hover:border-slate-600' : 'border-slate-200 bg-white hover:border-[#1E88E5]/50')
                          }`}
                        >
                          <div className={`w-5 h-5 rounded-md border-2 flex items-center justify-center flex-shrink-0 transition-all ${
                            isSelected ? 'bg-[#1E88E5] border-[#1E88E5]' : isDarkMode ? 'border-slate-500' : 'border-slate-300'
                          }`}>
                  ...
                  <div className="flex flex-col lg:flex-row lg:items-start justify-between gap-5">
                    ...
                    <div className="flex flex-row lg:flex-col items-center justify-between lg:items-end gap-4 mt-4 lg:mt-0">
                      ...
                      <div className="flex items-center gap-2">
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); setAssignTaskJob(job); }} className={`p-2.5 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-[#1E88E5]/40 text-[#3FA9F5]' : 'hover:bg-[#1E88E5]/10 text-[#1E88E5] hover:text-[#0D47A1]'}`} title="Assign Task"><FiClipboard className="w-5 h-5" /></motion.button>
                        <motion.button whileHover={{ scale: 1.1 }} whileTap={{ scale: 0.9 }} onClick={(e) => { e.stopPropagation(); handleEditJob(job); }} className={`p-2.5 rounded-lg transition-colors ${isDarkMode ? 'hover:bg-slate-700 text-slate-400' : 'hover:bg-[#1E88E5]/10 text-slate-500 hover:text-[#1E88E5]'}`} title="Edit"><FiEdit2 className="w-5 h-5" /></motion.button>
        ...
        {confirmDelete && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[70] flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm"
            ...
            <motion.div initial={{ scale: 0.9, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} exit={{ scale: 0.9, opacity: 0 }}
              onClick={(e) => e.stopPropagation()} className={`w-full max-w-md rounded-2xl p-6 ${isDarkMode ? 'bg-slate-800' : 'bg-white'} shadow-2xl`}>
              <div className="text-center">
                <div className="w-16 h-16 rounded-full flex items-center justify-center mx-auto mb-4 bg-red-100">
                  <FiTrash2 className="w-8 h-8 text-red-600" />
                </div>
                <h3 className={`text-xl font-bold mb-2 ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>Delete Position</h3>
                <p className={`text-sm mb-6 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>Are you sure you want to delete this position? This action cannot be undone.</p>
                <div className="flex gap-3">
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setConfirmDelete(null)}
                    className={`flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold ${isDarkMode ? 'bg-slate-700 text-slate-300' : 'bg-slate-100 text-slate-600'}`}>Cancel</motion.button>
                  <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => handleDeleteJob(confirmDelete)}
                    className="flex-1 px-4 py-2.5 rounded-xl text-sm font-semibold text-white bg-gradient-to-r from-red-500 to-rose-600 shadow-lg">Delete</motion.button>
                </div>
              </div>
            </motion.div>
          </motion.div>

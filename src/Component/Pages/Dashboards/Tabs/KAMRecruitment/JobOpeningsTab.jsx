  ...
  return (
    <div className="flex flex-col h-full bg-white relative animate-in fade-in slide-in-from-right duration-500">
      ...
      <div className="sticky top-0 bg-white/95 backdrop-blur-md border-b border-[#F4F3EF] px-8 py-6 flex items-center justify-between z-20">
        <div className="flex items-center gap-3">
          <Briefcase className="text-[#0D47A1]" size={22} />
          ...
          {isEditing ? (
            <div className="flex items-center gap-2">
              ...
              <button
                disabled={isSaving}
                onClick={handleSave}
                className="px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-[#0D47A1] hover:bg-[#0a3a82] transition-all flex items-center gap-2 shadow-md shadow-blue-500/10"
              >
          ...
          <div className="space-y-1.5 w-full">
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              {job.clientSource === 'sharepoint' && (
                <span className="inline-flex items-center gap-1.5 px-2 py-0.5 bg-emerald-50 text-emerald-600 rounded-lg text-[9px] font-black uppercase tracking-widest border border-emerald-100">
...
const JobOpeningsTab = ({ isDarkMode, notificationBell }) => {
  ...
  const fetchPositions = async () => {
    ...
    try {
      ...
      // Filter by assignedToId for KAM users
      if (isKAM) {
        ...
        if (!currentUserId) {
          try {
            ...
            console.error('Failed to parse token in fetchPositions:', e);
          }
        }
        if (currentUserId) {
  ...
  const handleBulkStatusUpdate = async (newStatus) => {
    ...
    try {
      const updatePromises = selectedJobs.map(id => {
        let payload;
        if (newStatus === 'Urgent') {
          payload = { status: 'Urgent', priority: 'Urgent' };
        } else if (newStatus === 'Medium') {
          payload = { status: 'Open', priority: 'Medium' };
        } else {
          payload = priorityValues.includes(newStatus)
            ? { priority: newStatus }
            : { status: newStatus };
        }
      ...
      setJobs(prev => prev.map(job =>
        selectedJobs.includes(job.id)
          ? { 
              ...job, 
              ...(newStatus === 'Urgent' 
                ? { status: 'Urgent', priority: 'Urgent' } 
                : newStatus === 'Medium' 
                  ? { status: 'Open', priority: 'Medium' } 
                  : priorityValues.includes(newStatus) 
                    ? { priority: newStatus } 
                    : { status: newStatus }
              ) 
            }
    ...
    <>
      {typeof document !== 'undefined' && createPortal(
        <AnimatePresence>
          {showFullPageForm && (
            <div 
              className="fixed inset-0 z-[1100] flex items-center justify-center p-4 bg-[#1A1A2E]/40 backdrop-blur-md transition-all duration-300 pointer-events-auto"
              onClick={handleBackToJobs}
            >
            <div 
              className="bg-white rounded-3xl w-full max-w-xl overflow-hidden shadow-2xl animate-in fade-in slide-in-from-bottom-8 duration-500"
            ...
            >
              ...
              {modalStep === 1 ? (
                    ...
                    <div className="space-y-1.5" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="space-y-3">
                        <div className="relative" ref={roleDropdownRef}>
                          ...
                          >
                            <span>{newJobForm.roleType === 'other' ? (otherRole ? `${otherRole} (Custom)` : 'Custom Role') : (newJobForm.roleType || "Select Role Category")}</span>
                            <ChevronDown size={16} className="text-[#0D47A1] opacity-50" />
                          </div>
                          ...
                          {showRoleDropdown && (
                            <div className="absolute z-50 w-full mt-2 rounded-2xl shadow-2xl bg-white border border-[#F4F3EF] p-3 space-y-2 max-h-60 overflow-y-auto">
                              ...
                              <div className="space-y-1">
                                ...
                                {roleTypes
                                  .filter(r => (r.role || '').toLowerCase().includes(roleSearchTerm.toLowerCase()))
                                  .map(r => (
                                    <button
                                      key={r.role}
                                      type="button"
                                      onClick={() => {
                                        setNewJobForm(f => ({ ...f, roleType: r.role }));
                                        setShowRoleDropdown(false);
                                      }}
                                      className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                        newJobForm.roleType === r.role
                                ...
                                <button
                                  ...
                                  className="w-full text-left px-4 py-2 rounded-xl text-xs font-bold text-blue-600 hover:bg-blue-50 transition-all"
                                >
                                  + Add other role...
                                </button>
                              </div>
                    ...
                    <div className="space-y-1.5" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="relative" ref={companyDropdownRef}>
                        <div
                          ...
                          className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] cursor-pointer flex justify-between items-center"
                        >
                          <span>{newJobForm.client || "Select Company"}</span>
                          <ChevronDown size={16} className="text-[#0D47A1] opacity-50" />
                        </div>
                          ...
                          <div className="absolute z-50 w-full mt-2 rounded-2xl shadow-2xl bg-white border border-[#F4F3EF] p-3 space-y-2 max-h-60 overflow-y-auto">
                            ...
                            <div className="space-y-1">
                              <button
                                type="button"
                                onClick={() => {
                                  setNewJobForm(f => ({ ...f, clientId: "", client: "" }));
                                  setShowCompanyDropdown(false);
                                }}
                                className="w-full text-left px-4 py-2.5 rounded-xl text-xs font-bold text-slate-400 hover:bg-blue-50/50 transition-all"
                              >
                              ...
                              {(clients && clients.length > 0 ? clients : [
                                { id: '00000000-0000-0000-0000-000000000000', displayName: 'Mabicons ERP (Internal)', companyName: 'Mabicons', name: 'Mabicons ERP (Internal)' },
                                { id: '11111111-1111-1111-1111-111111111111', displayName: 'Standard Partner', companyName: 'General Partner', name: 'Standard Partner' }
                              ])
                                .filter(c => (c.displayName || c.companyName || c.name || '').toLowerCase().includes(companySearchTerm.toLowerCase()))
                                .map(c => {
                                  ...
                                  return (
                                    <button
                                      ...
                                      type="button"
                                      onClick={() => {
                                        setNewJobForm(f => ({ ...f, clientId: c.id, client: name }));
                                        setShowCompanyDropdown(false);
                                      }}
                                      className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                        newJobForm.clientId === c.id
                                          ? 'bg-blue-50 text-[#1B4DA0]'
                                          : 'text-[#1A1A2E] hover:bg-blue-50/30'
                                      }`}
                                    >
                ...
                <div className="p-10 max-h-[75vh] overflow-y-auto custom-scrollbar space-y-8">
                  <div className="flex items-center gap-4 p-6 rounded-[32px] border" style={{ backgroundColor: '#ecfdf5', borderColor: '#d1fae5' }}>
                    <div className="w-12 h-12 rounded-2xl flex items-center justify-center text-white animate-bounce" style={{ backgroundColor: '#10b981', boxShadow: '0 10px 25px rgba(16, 185, 129, 0.2)' }}>
                      <CheckCircle size={28} />
                    </div>
                    <div>
                      <p className="text-lg font-bold leading-tight" style={{ color: '#064e3b' }}>Position Created Successfully</p>
                      <p className="text-sm font-medium mt-0.5" style={{ color: 'rgba(4, 120, 87, 0.7)' }}>We found {matchedResumes.length} matching resumes for &quot;{newJobForm.title}&quot;</p>
                    </div>
                  </div>

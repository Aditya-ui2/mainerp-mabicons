  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      ...
      >
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowAnalytics(!showAnalytics)}
            ...
            style={showAnalytics ? { background: 'linear-gradient(135deg, #8b5cf6, #7c3aed)' } : {}}>
            <FiBarChart2 className="w-3.5 h-3.5" /> Analytics
          </motion.button>
          {/* Export CSV */}
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={exportToCSV}
            className={`flex items-center gap-1.5 px-3 py-2.5 text-xs font-medium rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
          ...
          {/* Upload CVs */}
          <motion.button whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }} onClick={() => setShowUploadModal(true)}
            className={`flex items-center gap-1.5 px-3 py-2.5 text-xs font-medium rounded-xl transition-colors ${isDarkMode ? 'bg-slate-700 text-slate-300 hover:bg-slate-600' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            <FiUpload className="w-3.5 h-3.5" /> Upload CVs
          </motion.button>
          {/* Add Candidate */}
        ...
        {[
          ...
          return (
            <motion.div key={stat.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 * (i + 1) }} whileHover={{ scale: 1.02, y: -2 }}
              ...
              <div className="absolute -right-4 -top-4 w-24 h-24 opacity-10"><div className="w-full h-full rounded-full" style={{ backgroundColor: stat.color }}></div></div>
              <div className="relative flex items-start justify-between">
                ...
                <div className="p-4 rounded-xl" style={{ backgroundColor: stat.color, boxShadow: `0 10px 15px -3px ${stat.color}40` }}>
            ...
            {jobOpenings.map(job => {
              ...
              return (
                <motion.div key={job.id} whileHover={{ scale: 1.01 }}
                  className={`relative rounded-xl p-3.5 border transition-all cursor-pointer ${isFilled ? isDarkMode ? 'bg-emerald-900/20 border-emerald-700/30' : 'bg-emerald-50 border-emerald-200' : isDarkMode ? 'bg-slate-700/50 border-slate-600/50 hover:border-blue-500' : 'bg-slate-50 border-slate-200 hover:border-blue-300'}`}
                  onClick={() => { if (!isFilled) { setNewCandidate(prev => ({ ...prev, jobTitle: job.title, client: job.client })); setShowAddModal(true); } }}>
                  <div className="flex items-center justify-between">
                    <div className="flex-1 min-w-0"><p className={`text-sm font-semibold truncate ${isDarkMode ? 'text-white' : 'text-slate-800'}`}>{job.title}</p><p className={`text-xs mt-0.5 ${isDarkMode ? 'text-slate-400' : 'text-slate-500'}`}>{job.client}</p></div>
                    <span className={`text-xs font-bold px-2 py-1 rounded-full ${isFilled ? 'bg-emerald-500 text-white' : 'bg-blue-500 text-white'}`}>{job.filled}/{job.openings}</span>
                  </div>
                  <div className="flex items-center gap-3 mt-2">
                    <span className={`text-[10px] ${isDarkMode ? 'text-slate-500' : 'text-slate-400'}`}>{candidatesForJob} in pipeline</span>
                    {isFilled ? <span className="text-[10px] font-semibold text-emerald-500 flex items-center gap-1"><FiCheckCircle className="w-3 h-3" /> Filled</span> : <span className="text-[10px] font-semibold text-blue-500 flex items-center gap-1"><FiPlus className="w-3 h-3" /> Add Candidate</span>}
                  </div>
                  <div className={`w-full h-1.5 rounded-full mt-2 overflow-hidden ${isDarkMode ? 'bg-slate-600' : 'bg-slate-200'}`}>
                    <div className="h-full rounded-full transition-all" style={{ width: `${Math.min((job.filled / job.openings) * 100, 100)}%`, background: isFilled ? 'linear-gradient(90deg, #10b981, #059669)' : 'linear-gradient(90deg, #3b82f6, #6366f1)' }} />
                  </div>
                </motion.div>
              );
            })}
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide">
        {[
          { key: 'all', label: 'All', count: stats.total, color: '#8b5cf6', icon: FiUsers },
          { key: 'pending', label: 'Pending', count: stats.pending, color: '#f59e0b', icon: FiClock },
          { key: 'hold', label: 'On Hold', count: stats.onHold, color: '#6366f1', icon: FiAlertCircle },
          { key: 'approved', label: 'Approved', count: stats.approved, color: '#10b981', icon: FiCheckCircle },
          { key: 'rejected', label: 'Rejected', count: stats.rejected, color: '#ef4444', icon: FiXCircle },
          ...
          <motion.button key={key} whileHover={{ scale: 1.02 }} whileTap={{ scale: 0.98 }}
            ...
            className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap transition-all ${filterPipelineStatus === key ? 'text-white shadow-lg' : isDarkMode ? 'bg-slate-700/50 text-slate-300 hover:bg-slate-700' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}
            style={filterPipelineStatus === key ? { backgroundColor: color, boxShadow: `0 10px 15px -3px ${color}40` } : {}}>
            <Icon className="w-4 h-4" style={filterPipelineStatus === key ? { color: 'white' } : { color }} />
            {label}
            <span className={`px-2 py-0.5 rounded-full text-xs ${filterPipelineStatus === key ? 'bg-white/20 text-white' : isDarkMode ? 'bg-slate-600 text-slate-300' : 'bg-slate-200 text-slate-600'}`}>{count}</span>
          </motion.button>
        ))}
      </div>

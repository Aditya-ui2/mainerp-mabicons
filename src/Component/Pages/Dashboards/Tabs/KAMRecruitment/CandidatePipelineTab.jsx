...
import { getResumeBankResumes, getResumeRoleTypes, getAllCandidates, addCandidate as addCandidateAPI, updateCandidateStatus, updateCandidate, scheduleNewInterview, getAllRecruitmentPositions, uploadResumes, getSharePointCandidates, syncSharePointAll, getResumeDownloadUrl, BASE_URL } from '../../../service/api';
...
const CandidatePipelineTab = ({ isDarkMode, setActiveTab, quickAction, onQuickActionHandled, notificationBell }) => {
  ...
  const [kamLeavingReasonOption, setKamLeavingReasonOption] = useState("");

  const handleKamReasonChange = (val) => {
    setKamLeavingReasonOption(val);
    if (val === "Other") {
      setNewCandidate(prev => ({ ...prev, reasonForLeaving: "" }));
    } else {
      setNewCandidate(prev => ({ ...prev, reasonForLeaving: val }));
    }
  };
    ...
    try {
      ...
      const fullPool = [
        ...
        ...rawSP.map(c => ({
          ...
          resumeId: c.sharePointId || c.id || c._id
        }))
  ...
  const openEditModal = (candidate) => {
    const skillsString = Array.isArray(candidate.skills)
      ? candidate.skills.join(', ')
      : candidate.skills || '';

    const reason = candidate.reasonForLeaving || "";
    if (reason === "") {
      setKamLeavingReasonOption("");
    } else if (LEAVING_REASONS.includes(reason)) {
      setKamLeavingReasonOption(reason);
    } else {
      setKamLeavingReasonOption("Other");
    }

    const getRawVal = (val) => {
      if (!val) return '';
      return String(val).replace(/\s*(LPA|Years|Year)/g, '').trim();
    };
    ...
    setNewCandidate({
      ...
      experience: getRawVal(candidate.experience),
      currentCTC: getRawVal(candidate.currentCTC),
      expectedCTC: getRawVal(candidate.expectedCTC),
  ...
  const handleUpdateCandidate = async () => {
    if (!newCandidate.name || !newCandidate.email || !newCandidate.phone || !newCandidate.jobTitle) { toast.error('Please fill required fields (Name, Email, Phone Number, Job Title)'); return; }
    if (!validatePhone(newCandidate.phone)) { toast.error('Phone number must be exactly 10 digits'); return; }
    ...
    try {
      const formatVal = (val, type) => {
        ...
        if (!trimmed) return '';
        if (!isNaN(Number(trimmed)) && !String(trimmed).includes('LPA') && !String(trimmed).includes('Year')) {
      ...
      const updatePayload = {
        ...
        skills: skillsArr,
        experience: formatVal(newCandidate.experience, 'experience'),
        currentSalary: formatVal(newCandidate.currentCTC, 'salary'),
        expectedSalary: formatVal(newCandidate.expectedCTC, 'salary'),
        noticePeriod: newCandidate.noticePeriod || '30 days',
        positionId: newCandidate.positionId || null,
        clientId: newCandidate.clientId || null,
        currentJobRole: newCandidate.currentJobRole || '',
        currentDesignation: newCandidate.currentDesignation || '',
        hometown: newCandidate.hometown || '',
        preferredLocation: newCandidate.preferredLocation || '',
        relevantExperience: newCandidate.relevantExperience || '',
        highestQualification: newCandidate.highestQualification || '',
        reasonForLeaving: newCandidate.reasonForLeaving || ''
      };

      const res = await updateCandidate(editingCandidateId, updatePayload);
      if (res?.success) {
        toast.success(`Candidate ${newCandidate.name} updated successfully!`);
        fetchCandidates();
        closeAddModal();
      } else {
  ...
  const handleAddCandidate = async (stayOpen = false) => {
    ...
    if (!newCandidate.name || !newCandidate.email || !newCandidate.phone || !newCandidate.jobTitle) { alert('Please fill required fields (Name, Email, Phone Number, Job Title)'); return; }
    if (!validatePhone(newCandidate.phone)) { alert('Phone number must be exactly 10 digits'); return; }
    const skillsArr = (newCandidate.skills || '').split(',').map(s => s.trim()).filter(Boolean);
    ...
    try {
      ...
      const mapped = {
        ...
        rating: 0,
        experience: formatVal(newCandidate.experience, 'experience'),
        currentCTC: formatVal(newCandidate.currentCTC, 'salary'),
        expectedCTC: formatVal(newCandidate.expectedCTC, 'salary'),
        noticePeriod: newCandidate.noticePeriod || '30 days',
        skills: skillsArr,
      ...
      formData.append('experience', formatVal(newCandidate.experience, 'experience'));
      formData.append('currentSalary', formatVal(newCandidate.currentCTC, 'salary')); 
      formData.append('expectedSalary', formatVal(newCandidate.expectedCTC, 'salary'));
      ...
      formData.append('currentJobRole', newCandidate.currentJobRole || '');
      formData.append('currentDesignation', newCandidate.currentDesignation || '');
      formData.append('hometown', newCandidate.hometown || '');
      formData.append('preferredLocation', newCandidate.preferredLocation || '');
      formData.append('relevantExperience', newCandidate.relevantExperience || '');
      formData.append('highestQualification', newCandidate.highestQualification || '');
      formData.append('reasonForLeaving', newCandidate.reasonForLeaving || '');
        ...
        <div className="bg-white rounded-[32px] p-10 shadow-xl border border-[#F4F3EF]">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-10">
            ...
            <div className="bg-[#F8FAFF] rounded-[32px] p-8 border border-blue-50">
              ...
              <div className="bg-white border border-[#F4F3EF] rounded-[24px] p-6 mt-6">
                <h3 className="text-[10px] font-black uppercase tracking-[0.3em] text-[#9B9BAD] mb-4">Professional Profile</h3>
                <div className="grid grid-cols-2 gap-y-3 gap-x-4 text-left">
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Current Job Role</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.currentJobRole || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Current Designation</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.currentDesignation || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Relevant Experience</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.relevantExperience || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Highest Qualification</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.highestQualification || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Hometown Location</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.hometown || 'N/A'}</p>
                  </div>
                  <div>
                    <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Preferred Location</p>
                    <p className="text-xs font-bold text-[#1A1A2E]">{selectedCandidateDetail.preferredLocation || 'N/A'}</p>
                  </div>
                  {selectedCandidateDetail.reasonForLeaving && (
                    <div className="col-span-2">
                      <p className="text-[9px] uppercase font-black text-[#9B9BAD] mb-0.5">Reason for Leaving</p>
                      <p className="text-xs font-medium text-[#4B4B5E] leading-relaxed bg-[#FAFAF8] p-3 rounded-xl border border-[#F4F3EF] italic">
                        "{selectedCandidateDetail.reasonForLeaving}"
                      </p>
                    </div>
                  )}
              ...
              <div className="mt-8 flex gap-4">
                <button
                  onClick={() => {
                    openEditModal(selectedCandidateDetail);
                    setShowDetailSidebar(false);
                    setSelectedCandidateDetail(null);
                  }}
                  className="flex-1 py-4 bg-blue-600 hover:bg-blue-700 text-white font-bold rounded-2xl shadow-lg transition-all"
                >
                  Edit Profile
                </button>
                <button
                  onClick={() => holdCandidate(selectedCandidateDetail.id)}
                  className="flex-1 py-4 bg-amber-500 hover:bg-amber-600 text-white font-bold rounded-2xl shadow-lg transition-all"
                >
                  {selectedCandidateDetail.pipelineStatus === 'hold' ? 'Resume Pipeline' : 'Put on Hold'}
                </button>
                <button
                  onClick={() => {
                    setShowDetailSidebar(false);
                  ...
                  className="flex-1 py-4 bg-slate-500 hover:bg-slate-600 text-white font-bold rounded-2xl shadow-lg transition-all"
                >
                  Close
                </button>
              </div>
  ...
  return (
    <>
        ...
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col md:flex-row md:items-center md:justify-between mb-8 flex-wrap gap-4">
          ...
          <div className="flex gap-4 items-center flex-wrap">
            ...
            <button
              ...
              className="flex items-center gap-2.5 px-6 py-4 bg-white border-2 border-dashed border-[#1B4DA0]/30 text-[#1B4DA0] rounded-2xl text-[14px] font-bold hover:bg-blue-50/50 hover:border-[#1B4DA0] transition-all duration-300 active:scale-95 shadow-sm"
            >
                          ...
                          <div className="flex items-center justify-between pt-4 border-t border-dashed border-[#F4F3EF]">
                            ...
                            <div className="flex items-center gap-2 relative">
                              ...
                              <button 
                                onClick={(e) => {
                                  e.stopPropagation();
                                  setStageMenuId(stageMenuId === candidate.id ? null : candidate.id);
                                }}
                              ...
                              {stageMenuId === candidate.id && (
                                <>
                                  <div className="fixed inset-0 z-40 cursor-default" onClick={(e) => { e.stopPropagation(); setStageMenuId(null); }} />
                                  <div className="absolute right-0 bottom-8 bg-white border border-[#F4F3EF] rounded-2xl shadow-xl z-50 py-2 w-48 text-left animate-in fade-in slide-in-from-bottom-2 duration-200">
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setStageMenuId(null);
                                      ...
                                      className="w-full px-4 py-2.5 text-xs font-bold text-[#1A1A2E] hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2"
                                    >
                                      <FiEdit2 size={12} className="text-blue-500" /> Edit Profile
                                    </button>
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        setStageMenuId(null);
                                        openApproveModal(candidate.id);
                                      }}
                                      className="w-full px-4 py-2.5 text-xs font-bold text-[#1A1A2E] hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2"
                                    >
                                      <FiCalendar size={12} className="text-emerald-500" /> Schedule Interview
                                    </button>
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        holdCandidate(candidate.id);
                                      }}
                                      className="w-full px-4 py-2.5 text-xs font-bold text-[#1A1A2E] hover:bg-blue-50 hover:text-blue-600 transition-all flex items-center gap-2"
                                    >
                                      <FiPause size={12} className="text-amber-500" /> {candidate.pipelineStatus === 'hold' ? 'Resume Pipeline' : 'Put on Hold'}
                                    </button>
                                    <button
                                      onClick={(e) => {
                                        e.stopPropagation();
                                        openRejectModal(candidate.id);
                                      }}
                                      className="w-full px-4 py-2.5 text-xs font-bold text-red-600 hover:bg-red-50 transition-all flex items-center gap-2"
                                    >
                                      <FiXCircle size={12} className="text-red-500" /> Reject Candidate
                                    </button>
                                  </div>
                                </>
                              )}
              ...
              >
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button 
                    ...
                    className="flex items-center gap-2 px-4 py-2 bg-[#F4F3EF] text-[#1A1A2E] rounded-xl text-xs font-bold hover:bg-blue-50 hover:text-blue-600 transition-all"
                  >
                    <FiEye className="w-4 h-4" />
                    View Details
                  </button>
          ...
          <>
            ...
            >
              ...
              <div className="flex-1 p-8 space-y-8 overflow-y-auto custom-scrollbar pb-10">
                ...
                <div className="bg-[#FAFAF8] rounded-3xl border border-[#F4F3EF] p-6">
                  <h3 className="text-[10px] font-black text-[#1A1A2E] uppercase tracking-[3px] mb-4">Professional Details</h3>
                  <div className="grid grid-cols-2 gap-y-4 gap-x-6 text-left">
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Current Job Role</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.currentJobRole || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Current Designation</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.currentDesignation || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Relevant Experience</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.relevantExperience || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Highest Qualification</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.highestQualification || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Hometown Location</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.hometown || 'N/A'}</p>
                    </div>
                    <div>
                      <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Preferred Location</p>
                      <p className="text-sm font-bold text-[#1A1A2E] mt-0.5">{drawerCandidate.preferredLocation || 'N/A'}</p>
                    </div>
                    {drawerCandidate.reasonForLeaving && (
                      <div className="col-span-2">
                        <p className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-wider">Reason For Leaving</p>
                        <p className="text-sm font-medium text-[#4B4B5E] leading-relaxed mt-1 bg-white/60 p-3 rounded-2xl border border-[#F4F3EF] italic">
                          "{drawerCandidate.reasonForLeaving}"
                        </p>
                      </div>
                    )}
              ...
              <div className="sticky bottom-0 bg-white/90 backdrop-blur-md border-t border-[#F4F3EF] p-6 flex gap-3 z-20">
                <button
                  onClick={() => {
                    openEditModal(drawerCandidate);
                    setDrawerCandidate(null);
                  }}
                  className="flex-1 py-3.5 bg-blue-50 text-[#1B4DA0] hover:bg-blue-100 font-bold rounded-2xl border border-blue-200 transition-all text-xs uppercase tracking-wider"
                >
                  Edit Profile
                </button>
                <button
                  onClick={() => holdCandidate(drawerCandidate.id)}
                  className="flex-1 py-3.5 bg-amber-50 text-[#D97706] hover:bg-amber-100 font-bold rounded-2xl border border-amber-200 transition-all text-xs uppercase tracking-wider"
                >
                  {drawerCandidate.pipelineStatus === 'hold' ? 'Resume' : 'Hold'}
                </button>
                <button
                  onClick={() => {
                    openApproveModal(drawerCandidate.id);
                    setDrawerCandidate(null);
                  }}
                  className="flex-[1.5] py-3.5 bg-[#1B4DA0] text-white hover:bg-[#153e82] font-bold rounded-2xl transition-all text-xs uppercase tracking-wider shadow-lg shadow-blue-500/10"
                >
                  Schedule
                </button>
              </div>
          ...
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[9999] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[40px] w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col shadow-2xl">
              <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-[#1A1A2E] font-syne">{editingCandidateId ? 'Edit Candidate Profile' : 'Add New Candidate'}</h2>
                  <p className="text-[10px] font-bold text-[#9B9BAD] mt-1">{editingCandidateId ? 'Update Details' : 'Pipeline Registration'}</p>
                </div>
              ...
              <div className="flex-1 overflow-y-auto p-10 space-y-4 custom-scrollbar">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-4">
                    ...
                    <input type="text" placeholder="Phone Number *" value={newCandidate.phone} onChange={e => setNewCandidate({ ...newCandidate, phone: e.target.value.replace(/\D/g, '').slice(0, 10) })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-4">
                  <input type="text" placeholder="Current Job Role" value={newCandidate.currentJobRole} onChange={e => setNewCandidate({ ...newCandidate, currentJobRole: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <input type="text" placeholder="Current Designation" value={newCandidate.currentDesignation} onChange={e => setNewCandidate({ ...newCandidate, currentDesignation: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <input type="text" placeholder="Hometown Location" value={newCandidate.hometown} onChange={e => setNewCandidate({ ...newCandidate, hometown: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <input type="text" placeholder="Preferred Work Location" value={newCandidate.preferredLocation} onChange={e => setNewCandidate({ ...newCandidate, preferredLocation: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <input type="text" placeholder="Relevant Experience" value={newCandidate.relevantExperience} onChange={e => setNewCandidate({ ...newCandidate, relevantExperience: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <input type="text" placeholder="Highest Qualification" value={newCandidate.highestQualification} onChange={e => setNewCandidate({ ...newCandidate, highestQualification: e.target.value })} className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all" />
                  <div className="md:col-span-2 space-y-2">
                    <select
                      value={kamLeavingReasonOption}
                      onChange={e => handleKamReasonChange(e.target.value)}
                      className="w-full bg-[#F4F3EF] border-none rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:ring-2 focus:ring-blue-100 transition-all appearance-none cursor-pointer text-[#1A1A2E]"
                    >
                  ...
                  {kamLeavingReasonOption === "Other" && (
                    <div className="md:col-span-2 space-y-2 animate-in fade-in slide-in-from-left-4">
                      <textarea
                        placeholder="Describe the reason of leaving..."
                        value={newCandidate.reasonForLeaving}
                        onChange={e => setNewCandidate({ ...newCandidate, reasonForLeaving: e.target.value })}
                        className="w-full bg-[#F4F3EF] border-2 border-[#1B4DA0]/20 rounded-2xl px-6 py-4 text-sm font-bold outline-none focus:bg-white focus:border-[#1B4DA0] transition-all min-h-[80px]"
                      />
                    </div>
                  )}
                </div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        {allMatches.length > 10 && (
                          <button
                            ...
                            className="text-[10px] font-bold text-emerald-600 hover:underline uppercase tracking-widest flex items-center gap-1.5"
                          >
              ...
              <div className="px-10 py-8 border-t border-[#F4F3EF] bg-[#F8FAFF] flex gap-4">
                ...
                <button onClick={() => handleAddCandidate(false)} className="flex-[1.5] py-4 bg-[#1B4DA0] text-white rounded-2xl text-sm font-bold shadow-xl hover:bg-[#153e82] transition-all">
      ...
      <AnimatePresence>
        {showUploadModal && (
          <div className="fixed inset-0 bg-black/60 backdrop-blur-xl z-[9999] flex items-center justify-center p-4">
            <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} exit={{ opacity: 0, scale: 0.95 }} className="bg-white rounded-[40px] w-full max-w-2xl overflow-hidden flex flex-col shadow-2xl">
              <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between">
                <div>
                  <h2 className="text-2xl font-bold text-[#1A1A2E] font-syne text-left">Bulk Upload Resumes</h2>
                  <p className="text-[10px] font-bold text-[#9B9BAD] mt-1 text-left">Add Multiple Candidates At Once</p>
                </div>
                <button onClick={() => { setShowUploadModal(false); resetBulkUploadModal(); }} className="w-12 h-12 rounded-2xl bg-[#F4F3EF] text-[#6B6B7E] flex items-center justify-center hover:bg-red-50 hover:text-red-500 transition-all">
                  <FiX className="w-5 h-5" />
                </button>
              </div>
              <div className="flex-1 overflow-y-auto p-10 space-y-6 custom-scrollbar">
                ...
                <div className="space-y-4 text-left">
                  <label className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest pl-1">Upload Resume Files *</label>
                  <input 
                    type="file" 
                  ...
                  <div 
                    ...
                    className="border-2 border-dashed border-[#1B4DA0]/30 rounded-[32px] p-10 text-center hover:bg-blue-50/30 transition-all cursor-pointer group"
                  >
                      ...
                      {bulkUploadFiles.map((file, idx) => (
                        <div key={idx} className="flex items-center justify-between p-3.5 bg-[#F4F3EF]/60 rounded-2xl border border-[#F4F3EF] hover:border-blue-100 transition-all">
                          ...
                          <button 
                            ...
                            className="w-7 h-7 rounded-lg bg-white text-gray-400 hover:text-red-500 hover:bg-red-50 flex items-center justify-center transition-all border border-[#E8E7E2]"
                          >
                            <FiX size={14} />
                          </button>
                        </div>
              ...
              <div className="px-10 py-8 border-t border-[#F4F3EF] bg-[#F8FAFF] flex gap-4">
                <button onClick={() => { setShowUploadModal(false); resetBulkUploadModal(); }} className="flex-1 py-4 bg-white border-2 border-[#E8E7E2] rounded-2xl text-sm font-bold text-[#6B6B7E] hover:bg-gray-50 transition-all" disabled={bulkUploading}>Cancel</button>
                <button onClick={handleBulkResumeUpload} disabled={bulkUploading || bulkUploadFiles.length === 0 || !bulkUploadPositionId} className="flex-[1.5] py-4 bg-[#1B4DA0] text-white rounded-2xl text-sm font-bold shadow-xl hover:bg-[#153e82] transition-all disabled:opacity-50 flex items-center justify-center gap-2">
                  {bulkUploading ? (
                    <>

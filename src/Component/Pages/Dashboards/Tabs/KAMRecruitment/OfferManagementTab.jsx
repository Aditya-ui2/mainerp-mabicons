...
import { BASE_URL, getAllOffers, saveOffer, getOfferCandidateSuggestions, deleteOffer, saveOfferTemplate, getOfferTemplate, generateCandidateCredentials, shareCandidateCredentials, updateCandidateStatus, getAllClients, getAllRecruitmentPositions } from '../../../service/api';
import {
  OFFER_STATUS_COLORS,
  STATUS_ICONS,
  AVATAR_COLORS,
...
// --- Helper Functions ---
const formatMonthlySalary = (val) => {
  if (!val) return '₹0.00';
  const valStr = String(val).toLowerCase().trim();
  const cleanNum = parseFloat(valStr.replace(/[^\d.]/g, '')) || 0;
  if (cleanNum === 0) return '₹0.00';

  let monthlyVal = cleanNum;
  if (valStr.includes('lpa') || cleanNum < 100) {
    // It's in LPA, convert to monthly: (cleanNum * 100000) / 12
    monthlyVal = (cleanNum * 100000) / 12;
  }
  return `₹${Math.round(monthlyVal).toLocaleString('en-IN')}`;
};

const parseToMonthly = (val) => {
  if (!val) return 0;
  const valStr = String(val).toLowerCase().trim();
  const numericVal = parseFloat(valStr.replace(/[^\d.]/g, '')) || 0;
  if (numericVal === 0) return 0;

  if (valStr.includes('lpa')) {
    return (numericVal * 100000) / 12;
  }
  if (numericVal < 100) {
    // Heuristic: values under 100 are LPA (e.g. 3, 5, 12 LPA)
    return (numericVal * 100000) / 12;
  } else if (numericVal >= 100000) {
    // Heuristic: values >= 100,000 are annual CTC in full (e.g. 300000, 2400000)
    return numericVal / 12;
  } else {
    // Heuristic: values between 100 and 100,000 are monthly salary (e.g. 15000, 50000)
    return numericVal;
  }
};
...
/* ── Detail Drawer ── */
function OfferDetailDrawer({ offer, onClose, onEdit, onDelete, onStatusUpdate, onSaveInline, isDarkMode }) {
  ...
  useEffect(() => {
    if (offer) setEditedOffer(offer);
  }, [offer]);

  const handleSave = async () => {
    setIsSaving(true);
    await onSaveInline(editedOffer);
    setIsSaving(false);
    setIsEditing(false);
  };

  const handleShareCredentialsClick = async () => {
    try {
      setIsSharing(true);
      const targetId = offer.candidateId || offer.id;
      const res = await shareCandidateCredentials(targetId);
      if (res && res.success) {
        toast.success("Credentials sent to candidate's email successfully! ✉️");
      } else {
        toast.error(res.message || "Failed to share credentials via email");
      }
    } catch (e) {
      console.error(e);
      toast.error(e.message || "An error occurred while sharing credentials");
    } finally {
      setIsSharing(false);
    }
  };
            ...
            {isEditing ? (
              <>
                <button
                  onClick={handleSave}
                  disabled={isSaving}
                  className={`w-10 h-10 rounded-xl flex items-center justify-center transition-all shadow-sm ${isDarkMode ? 'bg-emerald-600 text-white hover:bg-emerald-500' : 'bg-emerald-500 text-white hover:bg-emerald-600'}`}
                  title="Save Details"
                >
                  {isSaving ? <RefreshCw size={16} className="animate-spin" /> : <CheckCircle2 size={16} />}
                </button>
                <button
                  onClick={() => { setIsEditing(false); setEditedOffer(offer); }}
                  className={`w-10 h-10 rounded-xl flex items-center justify-center border transition-all ${isDarkMode ? 'text-slate-400 border-slate-700 hover:bg-slate-800' : 'text-[#6B6B7E] border-[#F4F3EF] hover:bg-slate-50'}`}
        ...
        <div className="flex-1 p-10 space-y-12 overflow-y-auto custom-scrollbar">
          ...
          <div className="grid grid-cols-2 gap-x-8 gap-y-12 px-2">
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedOffer.candidateName || ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, candidateName: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  ...
                  onChange={(e) => setEditedOffer({ ...editedOffer, email: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  ...
                  onChange={(e) => setEditedOffer({ ...editedOffer, position: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedOffer.client || ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, client: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedOffer.currentCTC || ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, currentCTC: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
                <p className={`text-[15px] font-black ${isDarkMode ? 'text-white' : 'text-[#1A1A2E]'}`}>{formatMonthlySalary(offer.currentCTC)}</p>
              )}
            </div>
            <div>
              ...
              {isEditing ? (
                <input
                  type="text"
                  value={editedOffer.offeredCTC || ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, offeredCTC: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
                <p className={`text-[15px] font-black ${isDarkMode ? 'text-white' : 'text-[#1A1A2E]'}`}>{formatMonthlySalary(offer.offeredCTC)}</p>
              )}
            </div>
            <div>
              ...
              {isEditing ? (
                <input
                  type="date"
                  value={editedOffer.joiningDate ? editedOffer.joiningDate.split('T')[0] : ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, joiningDate: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
            ...
            <div>
              ...
              {isEditing ? (
                <input
                  type="date"
                  value={editedOffer.offerDate ? editedOffer.offerDate.split('T')[0] : ''}
                  onChange={(e) => setEditedOffer({ ...editedOffer, offerDate: e.target.value })}
                  className={`w-full bg-transparent border-b ${isDarkMode ? 'border-slate-700 text-white focus:border-blue-500' : 'border-slate-300 text-[#1A1A2E] focus:border-blue-600'} outline-none py-1 text-[15px] font-black`}
                />
              ) : (
          ...
          {offer.bgvStatus !== 'Not Started' && (
            <section className="text-left px-2">
              ...
              <div className={`p-6 rounded-[2rem] border-2 border-dashed relative overflow-hidden flex items-center justify-between ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-slate-50 border-slate-200'}`}>
                <div className="flex-1 space-y-4">
                  ...
                  <div className="flex flex-col gap-1 mt-2 text-left">
                    <span className="text-[9px] font-black text-[#9B9BAD] uppercase tracking-tighter">Candidate Login Link:</span>
                    <a href="https://erp.mabicons.com/candidate-login" target="_blank" rel="noopener noreferrer" className="text-[10px] font-bold text-blue-600 hover:underline break-all">
                      https://erp.mabicons.com/candidate-login
                    </a>
                  </div>
                  <div className="mt-3 flex items-center gap-2">
                    <button
                      onClick={handleShareCredentialsClick}
                      disabled={isSharing}
                      className="px-4 py-2.5 bg-[#1B4DA0] hover:bg-[#153e82] disabled:opacity-50 text-white rounded-xl text-[10px] font-black uppercase tracking-wider flex items-center gap-1.5 transition-all shadow-sm active:scale-95 cursor-pointer"
                    >
                      {isSharing ? 'Sharing...' : 'Share via Email'} <Send size={12} />
                    </button>
                  </div>
                </div>
              ...
              {/* Requested Re-uploads Notes */}
              {offer.kycDocuments && Object.keys(offer.kycDocuments).some(key => offer.kycDocuments[key]?.verified === false) && (
                <div className={`mt-4 p-5 rounded-[2rem] border ${isDarkMode ? 'bg-rose-955/20 border-rose-900/30' : 'bg-rose-50/50 border-rose-100/50'} space-y-3`}>
                  <div className="flex items-center gap-2">
                    <AlertCircle size={14} className="text-rose-500" />
                    <h4 className="text-[9px] font-black text-rose-500 uppercase tracking-[0.2em]">Requested Re-uploads</h4>
                  </div>
                  <div className="space-y-2">
                    {Object.keys(offer.kycDocuments).map(key => {
                      const doc = offer.kycDocuments[key];
                      if (doc?.verified === false) {
                        const getDocLabel = (k) => {
                          const docLabels = {
                            ...
                            semester_1: 'Semester 1 Marksheet',
                            semester_2: 'Semester 2 Marksheet',
                            semester_3: 'Semester 3 Marksheet',
                            semester_4: 'Semester 4 Marksheet',
                            semester_5: 'Semester 5 Marksheet',
                            semester_6: 'Semester 6 Marksheet',
                            semester_7: 'Semester 7 Marksheet',
                            semester_8: 'Semester 8 Marksheet',
                            degree: 'Degree Certificate',
                            payslip_1: 'Pay Slip (Month 1)',
                            payslip_2: 'Pay Slip (Month 2)',
                            payslip_3: 'Pay Slip (Month 3)',
                            bank_statement: 'Bank Statement',
                            appointment_letter: 'Appointment Letter',
                            relieving_letter: 'Relieving/Experience Letter'
                          };
                          return docLabels[k] || k.replace(/_/g, ' ').toUpperCase();
                        };
                        return (
                          <div key={key} className={`text-[11px] leading-relaxed border-l-2 pl-3 text-left ${isDarkMode ? 'border-rose-800' : 'border-rose-200'}`}>
                            <p className={`font-extrabold ${isDarkMode ? 'text-rose-400' : 'text-rose-900'}`}>{getDocLabel(key)}</p>
                            <p className={`font-bold ${isDarkMode ? 'text-rose-300/70' : 'text-rose-700/80'}`}>{doc.rejectionReason || 'Re-upload requested'}</p>
                          </div>
                        );
                      }
                      return null;
...
/* ══════════════════════════════════════════════════════ */
const OfferManagementTab = ({ isDarkMode, notificationBell }) => {
  ...
  const candidateSuggestionsRef = useRef(null);
  ...
  const loadSuggestions = async (searchVal) => {
    ...
    try {
      const response = await getOfferCandidateSuggestions(trimmed);
  ...
  const calculateHike = (current, offered) => {
    ...
    const currentVal = parseToMonthly(current);
    const offeredVal = parseToMonthly(offered);
  ...
  useEffect(() => {
    ...
    const fetchAllClients = async () => {
      try {
        const response = await getAllClients({ service: 'recruitment' });
  ...
  const handleSaveInlineOffer = async (updatedOffer) => {
    try {
      const payload = {
        ...updatedOffer,
      };
      ...
      if (res && res.success) {
        ...
        fetchOffers();
        setViewingOffer(res.data || updatedOffer);
      } else {
        toast.error(res?.message || "Failed to update offer");
      }
    } catch (err) {
      console.error("Save inline offer error:", err);
  ...
  useEffect(() => {
    const handleOutsideClick = (event) => {
      ...
      if (candidateSuggestionsRef.current && !candidateSuggestionsRef.current.contains(event.target)) {
        setShowCandidateSuggestions(false);
      }
    };
  ...
  const statCards = [
    {
      ...
      value: (() => {
        const total = offers.reduce((acc, current) => acc + parseToMonthly(current.offeredCTC), 0);
        return total >= 100000 ? `₹${(total / 100000).toFixed(2)}L` : `₹${Math.round(total).toLocaleString()}`;
      })(),
          ...
          <div className="bg-white rounded-[32px] border border-[#F4F3EF] shadow-sm overflow-hidden">
            <div className="overflow-x-auto custom-scrollbar">
              <div className="min-w-[1250px]">
                <div className="grid grid-cols-[40px_140px_minmax(180px,2fr)_minmax(120px,1.2fr)_minmax(160px,1.5fr)_150px_150px_50px] gap-4 px-6 py-4 border-b border-[#F4F3EF] bg-transparent">
                  ...
                  {["Offer Date", "Candidate", "Client", "Position", "BGV Protocol", "Verify Status", ""].map((h, i) => (
                  ...
                  <div className="divide-y divide-[#F4F3EF]">
                    {filteredOffers.map((offer) => (
                      <div
                        ...
                        className="grid grid-cols-[40px_140px_minmax(180px,2fr)_minmax(120px,1.2fr)_minmax(160px,1.5fr)_150px_150px_50px] gap-4 items-center px-6 py-3 border-b border-[#F4F3EF] last:border-0 hover:bg-[#F8FAFF] cursor-pointer transition-all group relative"
                      >
                            ...
                            <button
                              onClick={async (e) => {
                                ...
                                try {
                                  ...
                                  const loadingId = toast.loading("📡 BGV Id and Password...");
                        ...
                        {/* Verify Status */}
                        <div className="flex items-center justify-center py-1" onClick={e => e.stopPropagation()}>
                          {offer.bgvStatus === 'Verified' || isFullyVerified(offer) ? (
                          ...
                          ) : offer.bgvStatus === 'Re-upload Requested' ? (
                            <div className="flex flex-col items-center gap-1.5">
                              <span className="inline-flex items-center justify-center px-4 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-widest bg-rose-50 text-rose-600 border border-rose-200 shadow-sm opacity-90 cursor-default">
                                Action Required
                              </span>
                  ...
                  <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-gradient-to-r from-white to-[#F8FAFF]">
                    <div className="flex flex-col items-start text-left">
                      <h2 className="text-2xl font-bold text-[#1A1A2E] font-syne tracking-tight">
                        {editingOffer ? 'Refine Offer Details' : 'Generate Offer Letter'}
                      </h2>
                      ...
                      <div className="space-y-8">
                        <div className="space-y-6">
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                            <div className="relative" onClick={(e) => e.stopPropagation()} ref={candidateSuggestionsRef}>
                              ...
                              <input
                                ...
                                onFocus={(e) => {
                                  e.stopPropagation();
                                  const currentSearch = formData.candidateName.trim();
                                  if (currentSearch) {
                            ...
                            <div className="relative" onClick={(e) => e.stopPropagation()}>
                              ...
                              <div className="relative" ref={positionDropdownRef}>
                                <div
                                  ...
                                  className="w-full bg-[#F4F3EF]/60 border border-transparent rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:ring-2 focus:ring-blue-100 cursor-pointer flex justify-between items-center focus:bg-white focus:border-blue-100"
                                >
                                  <span>{formData.position || "Select Position"}</span>
                                  <ChevronDown size={16} className="text-[#9B9BAD]" />
                                </div>
                                  ...
                                  <div className="absolute z-50 w-full mt-2 rounded-2xl shadow-2xl bg-white border border-[#F4F3EF] p-3 space-y-2 max-h-60 overflow-y-auto">
                                    ...
                                    <div className="space-y-1">
                                      <button
                                        type="button"
                                        onClick={() => {
                                          setFormData(prev => ({ ...prev, position: "", client: "" }));
                                          setShowPositionDropdown(false);
                                        }}
                                      ...
                                      {[...new Set(allPositionsList.map(p => p.title))]
                                        .filter(title => title.toLowerCase().includes(positionSearchTerm.toLowerCase()))
                                        .map(title => (
                                          <button
                                            key={title}
                                            type="button"
                                            onClick={() => {
                                              const pos = allPositionsList.find(p => p.title === title);
                                              setFormData(prev => ({
                                                ...prev,
                                                position: title,
                                            ...
                                            className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${formData.position === title
                            ...
                            <div className="relative" onClick={(e) => e.stopPropagation()}>
                              ...
                              <div className="relative" ref={clientDropdownRef}>
                                <div
                                  ...
                                  className="w-full bg-[#F4F3EF]/60 border border-transparent rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:ring-2 focus:ring-blue-100 cursor-pointer flex justify-between items-center focus:bg-white focus:border-blue-100"
                                >
                                  <span>{formData.client || "Select Client"}</span>
                                  <ChevronDown size={16} className="text-[#9B9BAD]" />
                                </div>
                                      ...
                                      {activeClientNames
                                        ...
                                        .map(name => (
                                          <button
                                            ...
                                            className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${formData.client === name
                                                ? 'bg-blue-50 text-[#1B4DA0]'
                                                : 'text-[#1A1A2E] hover:bg-blue-50/30'
                                              }`}
                                          >
                        ...
                        <div className="pt-8 flex gap-4 border-t border-[#F4F3EF]">
                          ...
                          >
                            {isSavingOffer ? (
                              ...
                              <>
                                <Send size={16} />
                                {editingOffer ? 'Finalize Changes' : 'Execute Letter'}
                              </>
                            )}
                          </motion.button>
                        </div>
          ...
          <AnimatePresence>
            {viewingOffer && (
              <OfferDetailDrawer
                ...
                onSaveInline={handleSaveInlineOffer}

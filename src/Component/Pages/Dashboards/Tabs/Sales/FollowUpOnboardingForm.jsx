...
const FollowUpOnboardingForm = ({ isOpen, onClose, onComplete, initialData = null }) => {
  ...
  useEffect(() => {
    ...
    if (isOpen && !hasInitialized) {
      if (initialData) {
        const isCustom = !clients.some(c => c.companyName === initialData.clientName);
        setFormData({
          ...initialFormState,
          ...initialData,
  ...
  }, [initialData, isOpen, clients, hasInitialized]);
  ...
  const handleSubmit = async () => {
    ...
    const leadData = {
      ...
      status: 'Qualified', // Maps to Pending status in FollowUpsTab
    ...
    try {
      ...
      if (isEdit) {
        ...
        toast.success("Follow-up updated in database successfully! ✨");
      } else if (selectedLead) {
        const res = await updateLead(selectedLead.id, leadData);
        savedLead = res?.data || res?.lead;
        toast.success("Lead moved to Follow-up successfully! ✨");
      } else {
        const res = await createLead(leadData);
        savedLead = res?.data || res?.lead;
        toast.success("Follow-up added to database successfully! 🎉");
      }
      ...
      if (onComplete && savedLead) {
        onComplete({
          ...
          date: savedLead.lastContactDate || savedLead.createdAt?.split('T')[0] || formData.date,
          contactPerson: savedLead.contactPerson || 'N/A',
          status: ['qualified', 'follow up'].includes((savedLead.status || '').toLowerCase().trim()) ? 'Pending' : 'Completed'
        });
      }
      onClose();
    } catch (err) {
      console.error('Submission error:', err);
      toast.error(`Failed to ${isEdit || selectedLead ? 'update' : 'add'} follow-up in database`);
    } finally {
      setSubmitting(false);
    }
  };
          ...
          >
            ...
            <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-white sticky top-0 z-20">
              <div className="flex items-center gap-5">
                <div>
                  <h2 className="text-2xl font-bold text-[#1A1A2E] tracking-tight font-syne">{isEdit ? 'Edit Follow-up Details' : 'Add New Follow-up'}</h2>
                </div>
              </div>
            ...
            <div className="px-10 py-8 border-t border-[#F4F3EF] bg-white sticky bottom-0 z-20 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
              ...
              <div className="flex gap-4">
                ...
                >
                  {submitting ? 'Saving...' : (isEdit ? 'Save Changes' : 'Add Follow-up')} <FiCheck size={20} />
                </button>
              </div>
            </div>
          </motion.div>
...
export default FollowUpOnboardingForm;

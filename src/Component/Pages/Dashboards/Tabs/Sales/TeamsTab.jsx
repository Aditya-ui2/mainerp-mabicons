...
/* ══════════════════════════════════════════════════════ */
const TeamsTab = ({ isDarkMode, userRole = 'KAM' }) => {
  ...
  const fetchMembers = async (showSkeleton = false) => {
    ...
    try {
      const res = await getDepartmentTeamMembers('Sales');
      ...
      setMembers([
        { id: 'mock_tm1', name: 'Priyanshi Sharma', email: 'priyanshi@mabicons.com', role: 'Sales Head', status: 'Active' },
    ...
    if (userId) {
      const fetchClients = async () => {
        try {
          ...
          const backendClients = (res?.data || res?.clients || []).filter(c => c.stage !== 'Onboarding Complete');
          ...
          const allRes = await getAllClients();
          const allGlobal = (Array.isArray(allRes?.data) ? allRes.data : (Array.isArray(allRes?.clients) ? allRes.clients : (Array.isArray(allRes) ? allRes : [])))
            .filter(c => c.stage !== 'Onboarding Complete');
      ...
      <div className="bg-white rounded-[24px] p-2 border border-[#F4F3EF] shadow-sm flex items-center gap-3 mb-8 flex-wrap">
        ...
        <div className="relative group">
          ...
          >
            ...
            <option value="KAM - Sales">KAM - Sales</option>
            ...
            >
                ...
                <div className="min-w-0">
                  <div className="flex items-center gap-2">
                    ...
                    {(() => {
                      ...
                      return (
                        <div className={`px-2 py-0.5 rounded-full flex items-center gap-1.5 border shadow-sm ${isFromMe
              ...
              <div className="flex items-center justify-start gap-2 py-1" onClick={(e) => { e.stopPropagation(); e.nativeEvent?.stopImmediatePropagation?.(); }}>
                ...
                <button
                  ...
                  className={`p-2 rounded-lg transition-all inline-flex items-center justify-center ${m.phone && m.phone !== 'N/A'
                    ...
                    <div className="bg-[#FAFAF9] rounded-[48px] border border-[#F4F3EF] p-12 space-y-10 shadow-sm transition-all hover:shadow-md">
                      <div className="grid grid-cols-1 gap-y-10">
                        ...
                        <div className="flex items-center justify-between group">
                          ...
                          <span className="text-base font-black text-[#1A1A2E]">Sales</span>
                        </div>
                          ...
                          {userClients.map((client) => {
                            ...
                            return (
                              <button
                                ...
                                className={`px-5 py-4 rounded-2xl text-[12px] font-bold transition-all text-left flex items-center gap-3 group relative overflow-hidden ${isSelected
                      ...
                      <div className="grid grid-cols-3 gap-2 mt-2">
                        {['candidate', 'interview', 'client'].map((type) => (
                          <button
                            ...
                            className={`px-4 py-3 rounded-2xl text-[10px] font-black uppercase tracking-wider transition-all ${assignmentType === type
                      ...
                      if (window.confirm(`Remove ${selectedRowIds.length} member(s)?`)) {
                        try {
                          for (const id of selectedRowIds) {
                            await deleteDepartmentTeamMember(id);
                          }
                          setMembers(prev => prev.filter(m => !selectedRowIds.includes(m.id)));
                          setToast(`${selectedRowIds.length} member(s) removed`);
                          setSelectedRowIds([]);
                        } catch (err) {
                          console.error(err);
                          setToast('Failed to delete member(s)');
                        }
                        setTimeout(() => setToast(null), 3000);
                      }
                    }}

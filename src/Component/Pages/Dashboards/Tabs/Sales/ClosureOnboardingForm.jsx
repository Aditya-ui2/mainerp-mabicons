...
import { FiX, FiCheck, FiBriefcase, FiUser, FiCalendar, FiClock, FiFileText, FiSearch, FiChevronDown } from 'react-icons/fi';
import { toast } from 'react-hot-toast';
import { getAllLeads, createLead, updateLead } from '../../../service/api';
            ...
            >
              ...
              <div className="max-h-48 overflow-y-auto p-1 custom-scrollbar">
                {filteredOptions.length > 0 ? filteredOptions.map((opt, i) => (
                  ...
                  <div className="px-4 py-4 text-[12px] font-bold text-[#9B9BAD] text-center bg-slate-50/50 rounded-xl m-1">No results found matching "{searchTerm}"</div>
                )}
              </div>
            </motion.div>
          )}
...
const ClosureOnboardingForm = ({ isOpen, onClose, onComplete, initialData = null }) => {
      ...
      try {
        ...
        if (res) {
          if (Array.isArray(res.data?.leads)) {
            clientsList = res.data.leads;
          } else if (Array.isArray(res.data)) {
            clientsList = res.data;
          } else if (Array.isArray(res.leads)) {
            clientsList = res.leads;
          } else if (Array.isArray(res)) {
            clientsList = res;
          }
        }
        const filteredList = clientsList.filter(c => !['converted', 'lost'].includes((c.status || '').toLowerCase().trim()));
  ...
  const validateForm = () => {
    ...
    if (!finalClientName) {
      toast.error('Client / Lead Name is required');
      return false;
    }
    const required = ['date'];
  ...
  const handleSubmit = async () => {
    ...
    let rawAmount = 0;
    if (formData.time) {
      const cleaned = formData.time.toString().replace(/[^\d.]/g, '');
    ...
    const leadData = {
      ...
      contactPerson: formData.paymentStatus || 'Pending', 
    ...
    try {
      ...
      if (isEdit) {
        ...
        savedLead = res?.data || res?.lead;
        toast.success("Lead converted to Closure successfully! ✨");
      } else {
      ...
      if (onComplete && savedLead) {
        onComplete({
          ...
          date: savedLead.lastContactDate ? new Date(savedLead.lastContactDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
          contactPerson: savedLead.contactPerson || 'Pending',
          time: `₹${(savedLead.value || 0).toLocaleString()}`,
      ...
      toast.error(`Failed to ${isEdit ? 'update' : 'add'} closure`);
    } finally {
      setSubmitting(false);
    }
  };
          ...
          >
            ...
            <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-white sticky top-0 z-20">
              <div className="flex items-center gap-5">
                <div>
                  <h2 className="text-2xl font-bold text-[#1A1A2E] tracking-tight font-syne">{isEdit ? 'Edit Closure Details' : 'Add New Closure'}</h2>
                </div>
              </div>
            ...
            <div className="px-10 py-8 border-t border-[#F4F3EF] bg-white sticky bottom-0 z-20 flex items-center justify-between shadow-[0_-10px_30px_rgba(0,0,0,0.02)]">
              ...
              <div className="flex gap-4">
                ...
                >
                  {submitting ? 'Saving...' : (isEdit ? 'Save Changes' : 'Add Closure')} <FiCheck size={20} />
                </button>
              </div>
            </div>
          </motion.div>
...
export default ClosureOnboardingForm;

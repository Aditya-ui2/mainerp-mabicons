...
const ClientsTab = ({ notificationBell }) => {
  ...
  const filteredClients = clients.filter(c => {
    ...
    const isOpsOrBoth = !type || type.includes('operation') || type.includes('operations');
    if (!isOpsOrBoth) return false;
                  ...
                  {filteredClients.map((client) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="px-8 py-4 text-left">
                          <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-xl text-[11px] uppercase tracking-widest font-black ${['active', 'accepted'].includes((client.status || 'Active').toLowerCase().trim())
                              ? 'bg-[#10B981]/10 text-[#10B981] border border-[#10B981]/20'
                              : 'bg-slate-100 text-slate-600 border border-slate-200'
                            }`}>
                            <div className={`w-1.5 h-1.5 rounded-full ${['active', 'accepted'].includes((client.status || 'Active').toLowerCase().trim())
                                ? 'bg-[#10B981]'
                                : 'bg-slate-400'
                              }`} />

...
import { createTeamLeader, createEmployee, getAdminHierarchy, deleteEmployee, deleteTeamLeader, deleteTeamLeaderWithReassignment, deleteTeamLeaderAndPromoteEmployee, updateEmployee, resetEmployeePassword } from '../../../service/api';
import { jwtDecode } from "jwt-decode";
import { motion, AnimatePresence } from 'framer-motion';
import { FiPlus, FiCheck, FiX, FiChevronDown, FiChevronRight, FiInfo, FiFileText, FiUpload, FiMail, FiUsers, FiBriefcase, FiTarget, FiZap, FiPhone, FiSearch, FiRefreshCw, FiCheckCircle, FiEdit2, FiCamera, FiEye, FiTrash, FiUser, FiLock, FiDownload } from 'react-icons/fi';
...
const TeamTabs = ({ isDarkMode, notificationBell }) => {
  ...
  const [selectedDocsMember, setSelectedDocsMember] = useState(null);
  ...
  const handleFileChange = (e, docId, docLabel) => {
    ...
    if (file) {
      ...
      reader.onload = (event) => {
        ...
        toast.success(`${docLabel || 'File'} Selected`);
      };
  ...
  const viewExistingMemberDoc = (docId) => {
    const docData = selectedDocsMember?.documents?.[docId];
    if (!docData) {
  ...
  const downloadExistingMemberDoc = (docId) => {
    const docData = selectedDocsMember?.documents?.[docId];
    if (!docData) {
      toast.error('No document file to download');
      return;
    }
    ...
    const name = (typeof docData === 'object' && docData.name) ? docData.name : `document_${docId}`;
  ...
  useEffect(() => {
    if (selectedMember) {
      const emailLower = (selectedMember.email || '').toLowerCase().trim();
      const blockedEmails = JSON.parse(localStorage.getItem('blocked_emails') || '[]');
      const isMemberBlocked = blockedEmails.includes(emailLower) || selectedMember.status === 'Inactive' || selectedMember.status === 'Blocked';
      setIsBlocked(isMemberBlocked);
    } else {
      setIsBlocked(false);
    }
  }, [selectedMember]);
  ...
  const fetchHierarchy = async () => {
    ...
    try {
      ...
      if (token) {
        ...
        id = decoded.id || decoded._id || decoded.adminId || decoded.userId;
        role = decoded.role || decoded.userType || '';
        name = decoded.name || '';
      }
    } catch (e) {
      console.warn('jwtDecode error:', e);
    }

    // Local storage fallbacks
    id = id || localStorage.getItem('userId') || '';
    role = role || localStorage.getItem('userRole') || localStorage.getItem('userType') || '';
      ...
      setOrgChart({
        ...
        children: [
          ...
          },
          {
            ...
            phone: '+91 7654321098',
            children: [
              { name: 'Priyanshi', id: 'mock_kam1', employeeId: 'MAB-004', email: 'priyanshi@mabicons.com', role: 'KAM', designation: 'KAM Associate', department: 'Recruitment', status: 'Active', joiningDate: '2023-01-15', phone: '+91 6543210987', plainPassword: 'mock_password1' },
              { name: 'Manju', id: 'mock_kam2', employeeId: 'MAB-005', email: 'manju@mabicons.com', role: 'KAM', designation: 'KAM Associate', department: 'Recruitment', status: 'Active', joiningDate: '2023-02-20', phone: '+91 5432109876', plainPassword: 'mock_password2' },
              { name: 'Jyoti', id: 'mock_kam3', employeeId: 'MAB-006', email: 'jyoti@mabicons.com', role: 'KAM', designation: 'KAM Associate', department: 'Recruitment', status: 'Active', joiningDate: '2023-03-25', phone: '+91 4321098765', plainPassword: 'mock_password3' }
            ]
          }
        ]
      });
  ...
  const handleUpdateCredentials = async () => {
    const originalBlocked = selectedMember.status === 'Inactive' || selectedMember.status === 'Blocked';
    ...
    try {
      ...
      // Save password override and status block in localStorage for development/mock credentials
      if (selectedMember.email) {
        if (credentialPassword) {
          const customPasswords = JSON.parse(localStorage.getItem('custom_passwords') || '{}');
          customPasswords[selectedMember.email.toLowerCase().trim()] = credentialPassword;
      ...
      if (String(memberId).startsWith('mock_')) {
        // Fallback for development mock members
        await new Promise(resolve => setTimeout(resolve, 800));
        toast.success('Credentials updated successfully (Development Mode)');
      } else {
        await resetEmployeePassword(memberId, credentialPassword || undefined, isBlocked ? 'Inactive' : 'Active');
        toast.success('Credentials updated successfully');
      }

      // Check if self-updating
      const loggedInToken = localStorage.getItem('token');
      let isSelf = false;
      if (loggedInToken) {
        try {
          const decoded = jwtDecode(loggedInToken);
          const loggedInEmail = (decoded.email || '').toLowerCase().trim();
          const targetEmail = (selectedMember.email || '').toLowerCase().trim();
          const loggedInId = decoded.id || decoded._id || decoded.adminId || decoded.userId;
          const targetId = selectedMember._id || selectedMember.id;
          if ((loggedInEmail && loggedInEmail === targetEmail) || (loggedInId && loggedInId === targetId)) {
            isSelf = true;
          }
        } catch (e) { }
      }
      ...
      if (isSelf) {
        toast.success('You have updated your own credentials. Logging out...');
        localStorage.clear();
        setTimeout(() => {
    ...
    try {
      if (userRole === 'TeamLeader') {
        ...
        try {
          ...
          console.warn("API failed, falling back to mock creation in development:", apiErr);
          response = {
            employee: {
        ...
        if (newMember.role === 'leader') {
          const token = localStorage.getItem('token');
          let adminId = 'mock_admin_id';
          if (token) {
            try {
              const decoded = jwtDecode(token);
              adminId = decoded.id || decoded._id || decoded.adminId;
            } catch (errDec) { }
          }
          ...
          let response;
          try {
            response = await createTeamLeader(teamLeaderData);
          } catch (apiErr) {
            console.warn("API failed, falling back to mock creation in development:", apiErr);
            response = {
              teamLeader: {
                _id: `mock_tl_${Date.now()}`,
          ...
          newOrgChart.children.push({
            ...
            phone: response.teamLeader.phone,
            role: 'Team Leader',
            designation: 'Lead Executive',
            department: newMember.department === 'other' ? newMember.otherDepartment : newMember.department,
            status: 'Active',
            joiningDate: new Date().toISOString().split('T')[0],
            children: []
          });
          ...
          if (leaderNode) {
            ...
            leaderNode.children.push({
              ...
              email: response.employee.email,
              phone: response.employee.phone,
              role: newMember.role === 'other' ? newMember.otherRole : newMember.role,
              designation: 'Associate',
              department: newMember.department === 'other' ? newMember.otherDepartment : newMember.department,
              status: 'Active',
              joiningDate: new Date().toISOString().split('T')[0]
  ...
  const handleDeleteMember = async () => {
    ...
    try {
      ...
      // Save removed email to local storage block list for mock testing
      if (memberToDelete.email) {
        const blocked = JSON.parse(localStorage.getItem('blocked_emails') || '[]');
        if (!blocked.includes(memberToDelete.email.toLowerCase().trim())) {
          blocked.push(memberToDelete.email.toLowerCase().trim());
          localStorage.setItem('blocked_emails', JSON.stringify(blocked));
        }
      }
      ...
      if (memberToDelete.children) {
        ...
        try {
          ...
          console.warn("Delete API failed, removing locally in dev mode:", apiErr);
        }
      }
  ...
  const renderCustomNode = ({ nodeDatum, toggleNode, deleteNode }) => {
    ...
    const avatarUrl = nodeDatum.avatar || nodeDatum.picture;
    const hasImage = avatarUrl && (avatarUrl.startsWith('data:image') || avatarUrl.startsWith('http://') || avatarUrl.startsWith('https://'));
    const nodeId = nodeDatum.id || nodeDatum._id || 'root';
      ...
      <g className="node-group transition-all duration-300">
        ...
        <defs>
          <clipPath id={`clip-${nodeId}`}>
            <rect x={-95} y={-20} width={40} height={40} rx={10} />
          </clipPath>
        </defs>
        ...
        {hasImage ? (
          ...
          <g>
            <rect x={-95} y={-20} width={40} height={40} rx={10} fill="#F0F4FF" />
            <text
              x={-75}
              y={4}
              textAnchor="middle"
            ...
            >
              {(nodeDatum.name || 'U').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()}
            </text>
          </g>
        )}
  ...
  const filteredMembers = flattenedMembers.filter(member => {
    ...
    // Explicitly hide any removed/inactive/deleted members
    if (memberStatus === 'inactive' || memberStatus === 'deleted' || memberStatus === 'removed') {
      return false;
    }
    ...
    <div className="space-y-8 animate-in fade-in duration-500" style={{ fontFamily: "'Calibri', sans-serif" }}>
      ...
      <div className="bg-white rounded-[32px] shadow-sm border border-[#F4F3EF] overflow-hidden relative">
        <div className="overflow-x-auto min-h-[300px]">
          {/* Grid Header */}
          <div className="grid grid-cols-[40px_350px_250px_300px_350px_60px] gap-8 px-8 py-4 border-b border-[#F4F3EF] bg-transparent min-w-[1400px]">
          ...
          {filteredMembers.length === 0 ? (
            ...
            filteredMembers.map((member) => (
              <div
                ...
                className="grid grid-cols-[40px_350px_250px_300px_350px_60px] gap-8 items-center px-8 py-3 border-b border-[#F4F3EF] last:border-0 hover:bg-[#F8FAFF] cursor-pointer transition-all group min-w-[1400px]"
              >
                {/* Checkbox */}
                ...
                <div className="flex items-center gap-4 min-w-0 py-1">
                  <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-blue-50 to-[#F4F3EF] flex items-center justify-center text-[#1B4DA0] text-sm font-black border border-[#F4F3EF] group-hover:scale-105 transition-transform shrink-0 overflow-hidden">
                    {member.avatar && (member.avatar.startsWith('data:image') || member.avatar.startsWith('http://') || member.avatar.startsWith('https://')) ? (
                      <img src={member.avatar} alt="Profile" className="w-full h-full object-cover" />
                    ) : member.picture && (member.picture.startsWith('data:image') || member.picture.startsWith('http://') || member.picture.startsWith('https://')) ? (
                      <img src={member.picture} alt="Profile" className="w-full h-full object-cover" />
                    ) : (
                ...
                {/* Contact */}
                <div className="flex items-center justify-start gap-3 py-1" onClick={(e) => e.stopPropagation()}>
                  <span className="text-[13px] font-medium text-[#64748b] truncate text-left">{member.email}</span>
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 bg-[#F4F3EF] text-[#9B9BAD] hover:text-[#1B4DA0] hover:bg-blue-50 rounded-lg transition-all"
      ...
      {/* Member Profile & Access Control Modal */}
      {createPortal(
        <AnimatePresence>
          {selectedMember && (
              ...
              >
                ...
                <div className="px-8 py-6 border-b border-[#F4F3EF] flex items-center justify-between bg-gradient-to-r from-blue-50/50 to-white">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="text-left">
                      <h3 className="text-[16px] font-bold text-[#1A1A2E]" style={{ fontFamily: "'Syne', sans-serif" }}>Employee Profile</h3>
                      <p className="text-[11px] font-medium text-[#6B6B7E] uppercase tracking-widest mt-0.5">{selectedMember.name}</p>
                    </div>
                  </div>
                  <button
                    onClick={() => setSelectedMember(null)}
                ...
                <div className="p-8 space-y-6 bg-[#FAFAF8] overflow-y-auto max-h-[70vh] custom-scrollbar">
                  ...
                  <div className="bg-white rounded-3xl p-6 border border-[#E5E7EB] shadow-sm flex flex-col items-center text-center relative overflow-hidden">
                    ...
                    {/* Profile Image / Initials */}
                    <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-blue-500 to-[#1B4DA0] text-white flex items-center justify-center text-xl font-black shadow-md mb-3 overflow-hidden">
                      {selectedMember.avatar && (selectedMember.avatar.startsWith('data:image') || selectedMember.avatar.startsWith('http://') || selectedMember.avatar.startsWith('https://')) ? (
                        <img src={selectedMember.avatar} alt="Profile" className="w-full h-full object-cover" />
                      ) : selectedMember.picture && (selectedMember.picture.startsWith('data:image') || selectedMember.picture.startsWith('http://') || selectedMember.picture.startsWith('https://')) ? (
                        <img src={selectedMember.picture} alt="Profile" className="w-full h-full object-cover" />
                      ) : (
                        (selectedMember.name || 'U').split(' ').map(n => n[0]).join('').slice(0, 2).toUpperCase()
                      )}
                    </div>

                    <h4 className="text-lg font-bold text-[#1A1A2E]">{selectedMember.name}</h4>
                    <p className="text-xs font-semibold text-[#64748B] mt-0.5 uppercase tracking-wider">{selectedMember.role || 'Associate'}</p>
                    ...
                    <div className="grid grid-cols-2 gap-4 w-full text-left text-xs">
                      <div>
                        ...
                        <p className="font-semibold text-slate-800 truncate mt-0.5">{selectedMember.email}</p>
                      </div>
                      <div>
                        <p className="text-[10px] font-bold text-[#9B9BAD] uppercase tracking-wider">Phone</p>
                        <p className="font-semibold text-slate-800 mt-0.5">{selectedMember.phone || '+91 98765 43210'}</p>
                      </div>
                    </div>
                  </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="space-y-2 text-left">
                        ...
                        <input
                          ...
                          value={(() => {
                            if (!selectedMember || !selectedMember.email) return 'mabicons123';
                            const emailLower = selectedMember.email.toLowerCase().trim();
                            const customPasswords = JSON.parse(localStorage.getItem('custom_passwords') || '{}');
                            ...
                            return customPasswords[emailLower] || selectedMember.plainPassword || 'mabicons123';
                          })()}
                          readOnly
                        />
                      </div>
                      ...
                      <div className="space-y-2 text-left">
                        ...
                        <div className="relative">
                          ...
                          <button
                            ...
                            disabled={isLoading || !credentialPassword}
                      ...
                      {/* Documents Cabinet Section */}
                      <div className="space-y-4 pt-6 border-t border-[#F4F3EF]">
                        <div className="flex items-center justify-between">
                          <h5 className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest ml-1 text-left">Documents Cabinet</h5>
                          <button
                            type="button"
                            onClick={() => {
                              setSelectedDocsMember({ ...selectedMember, documents: selectedMember.documents || {} });
                              setIsViewDocsModalOpen(true);
                            }}
                            className="text-[10px] font-black text-[#1B4DA0] hover:text-[#0D47A1] uppercase tracking-widest transition-all"
                          >
                            Manage Docs
                          </button>
                        </div>
                        ...
                        <div className="grid grid-cols-2 gap-3">
                          {[
                            { label: 'PAN Front', id: 'pan_front' },
                            { label: 'PAN Back', id: 'pan_back' },
                            { label: 'Aadhaar Front', id: 'aadhaar_front' },
                            { label: 'Aadhaar Back', id: 'aadhaar_back' },
                            { label: '10th Marksheet', id: '10th_marksheet' },
                            { label: '12th Marksheet', id: '12th_marksheet' }
                          ].map((doc) => {
                            const hasDoc = selectedMember?.documents?.[doc.id];
                            return (
                              <div key={doc.id} className="flex items-center justify-between p-3 bg-white border border-[#E5E7EB] rounded-2xl shadow-sm">
                                <div className="flex items-center gap-2 truncate text-left">
                                  <FiFileText className={hasDoc ? "text-emerald-500 shrink-0" : "text-gray-400 shrink-0"} size={16} />
                                ...
                                {hasDoc ? (
                                  <div className="flex items-center gap-1">
                                    <button
                                      ...
                                      className="p-1.5 rounded-lg text-[#1B4DA0] hover:bg-blue-50 transition-all"
                                      title="View"
                                    >
                                      <FiEye size={14} />
                                    </button>
                                    <button
                                      type="button"
                                      onClick={() => downloadExistingMemberDoc(doc.id)}
                                  ...
                                  <span className="text-[9px] font-bold text-amber-500 uppercase tracking-wider px-1.5 py-0.5">Missing</span>
                                )}
                              </div>
                            );
                          })}
                ...
                <div className="px-8 py-6 border-t border-[#F4F3EF] bg-white flex items-center justify-end gap-3">
                  ...
                  <button
                    onClick={handleUpdateCredentials}
                    disabled={isLoading}
                    className="px-6 py-3 rounded-2xl text-[12px] font-bold text-white bg-[#0D47A1] hover:bg-[#0a3a82] transition-all shadow-lg shadow-blue-500/20 active:scale-95 flex items-center gap-2 disabled:opacity-50"
                  >
                    {isLoading ? 'Updating...' : <><FiCheck size={16} /> Update Access</>}
                  </button>
                </div>
              </motion.div>
            </div>
                          ...
                          {[
                            ...
                            <div key={doc.id} className="space-y-2">
                              ...
                              <div className="group relative">
                                  ...
                                  <div className="flex items-center gap-3">
                                    ...
                                    <div className="text-left">
                                      <p className="text-[11px] font-bold text-[#1A1A2E]">
                                        {newMember.documents?.[doc.id] ? newMember.documents[doc.id].name : 'Select File'}
                                      </p>
                                      <p className="text-[9px] font-medium text-[#9B9BAD]">
                                        {newMember.documents?.[doc.id] ? 'File selected' : 'PDF, JPG (Max 2MB)'}
                                      </p>
                                    </div>
                                  </div>
                                ...
                                <input type="file" className="absolute inset-0 opacity-0 cursor-pointer" onChange={(e) => handleFileChange(e, doc.id, doc.label)} />
                              </div>
                            </div>
                          ))}
                        </div>
          ...
          >
            ...
            <div className="px-10 py-6 border-b border-[#F4F3EF] flex items-center justify-between bg-white sticky top-0 z-10">
              <div className="flex flex-col text-left">
                ...
                <p className="text-[11px] font-medium text-[#9B9BAD]">Member: <span className="text-[#1B4DA0] font-bold">{selectedDocsMember?.name}</span></p>
              </div>
              <button
                onClick={() => {
                  setIsViewDocsModalOpen(false);
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {[
                      ...
                      <div key={doc.id} className="p-4 bg-[#F8FAFF] rounded-2xl border border-[#E2E8F0] flex items-center justify-between group hover:border-[#1B4DA0]/30 transition-all">
                        <div className="flex items-center gap-4">
                          <div className={`w-12 h-12 rounded-xl flex items-center justify-center transition-all ${selectedDocsMember?.documents?.[doc.id] ? 'bg-emerald-100 text-emerald-600' : 'bg-white text-[#C5C5D2] shadow-sm'}`}>
                            <FiFileText size={24} />
                          </div>
                          <div className="text-left">
                            ...
                            <p className="text-[10px] font-medium text-[#9B9BAD] max-w-[150px] truncate">
                              {selectedDocsMember?.documents?.[doc.id] ? (selectedDocsMember.documents[doc.id].name || 'File Uploaded') : 'No file chosen'}
                            </p>
                          </div>
                        </div>
                        <div className="flex items-center gap-2">
                          {selectedDocsMember?.documents?.[doc.id] ? (
                            <>
                              <button
                                type="button"
                                onClick={() => viewExistingMemberDoc(doc.id)}
                              ...
                              <button
                                type="button"
                                onClick={() => downloadExistingMemberDoc(doc.id)}
                                className="w-9 h-9 rounded-lg bg-white text-emerald-600 hover:bg-emerald-50 transition-all flex items-center justify-center shadow-sm border border-emerald-100"
                              >
                                <FiDownload size={16} />
                              </button>
                            </>
                ...
                <button
                  onClick={async () => {
                    ...
                    try {
                      ...
                      if (!String(memberId).startsWith('mock_')) {
                        await resetEmployeePassword(memberId, newPassword, selectedMember.status || 'Active');
                      }

                      // Save custom password in localStorage
                      if (selectedMember.email) {
                        const customPasswords = JSON.parse(localStorage.getItem('custom_passwords') || '{}');
                        customPasswords[selectedMember.email.toLowerCase().trim()] = newPassword;
                        localStorage.setItem('custom_passwords', JSON.stringify(customPasswords));
                      }

                      toast.success('Password updated successfully!');
                      setIsResetPassModalOpen(false);
                      setSelectedMember(prev => ({ ...prev, plainPassword: newPassword }));
                      await fetchHierarchy();
                    } catch (err) {
              ...
              <div className="grid grid-cols-7 gap-2">
                {(() => {
                  ...
                  for (let d = 1; d <= daysInMonth; d++) {
                    ...
                    const activeHours = 8 + (d % 2);
                    const activeMins = (d * 7) % 60;
                    const mockCheckIn = `09:${String((d * 3) % 60).padStart(2, '0')} AM`;
                    const mockCheckOut = `0${9 + activeHours - 12}:${String((d * 3 + activeMins) % 60).padStart(2, '0')} PM`;
                    ...
                    days.push(
                      <div
                        ...
                        title={
                          isPresent
                            ? `Status: Present\nCheck-in: ${mockCheckIn}\nCheck-out: ${mockCheckOut}\nActive Time: ${activeHours}h ${activeMins}m`
                            : isAbsent
                              ? `Status: Absent`
                              : undefined
                        }
                      >
                        ...
                        {isPresent && (
                          <div className="flex flex-col items-center mt-0.5 w-full px-1">
                            <span className="text-[8px] font-extrabold text-[#2E7D32] tracking-tighter leading-none mb-0.5">

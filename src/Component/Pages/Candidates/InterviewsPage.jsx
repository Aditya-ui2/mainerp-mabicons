...
export default function InterviewsPage({ notificationBell }) {
  ...
  const [recruitmentClients, setRecruitmentClients] = useState([]);
  ...
  const [showPositionSearchDropdown, setShowPositionSearchDropdown] = useState(false);
  const [positionSearchTerm, setPositionSearchTerm] = useState('');

  const clientSearchDropdownRef = useRef(null);
  const positionSearchDropdownRef = useRef(null);

  useEffect(() => {
    const handleOutsideClick = (event) => {
      if (clientSearchDropdownRef.current && !clientSearchDropdownRef.current.contains(event.target)) {
        setShowClientSearchDropdown(false);
      }
      if (positionSearchDropdownRef.current && !positionSearchDropdownRef.current.contains(event.target)) {
        setShowPositionSearchDropdown(false);
      }
    };
    ...
    try {
      ...
      if (candRes.success) {
        setCandidates(candRes.data.map(c => ({
          ...
          positionId: c.positionId || c.position?.id || "",
      ...
      // Fetch Recruitment Clients
      try {
        const clientRes = await getRecruitmentClients();
        if (clientRes && clientRes.success) {
          setRecruitmentClients(clientRes.data || []);
        }
      } catch (err) {
        console.error("Failed to fetch recruitment clients:", err);
      }

      // Fetch Recruitment Positions
      try {
        const posRes = await getAllRecruitmentPositions();
        if (posRes && posRes.success) {
          setRecruitmentPositions(posRes.data || []);
        }
      } catch (err) {
      ...
      if (erpRes.success) {
        erpRes.data.forEach(i => {
          if (i.evaluation && Object.keys(i.evaluation).length > 0) {
            let rating = i.evaluation.overallRating || i.evaluation.rating || 0;
            if (rating > 5) {
              rating = Math.round(rating / 2);
            }
            feedbackMap[i.id || i._id] = {
              rating,
              note: i.evaluation.notes || i.evaluation.comment || ""
            };
          }
        });
  ...
  const handleScheduleAgain = (interview) => {
    ...
    setInterviewForm({
      ...
      candidateEmail: interview.raw?.candidate?.email || interview.candidateEmail || "",
      ...
      mode: interview.type === "In-Person" ? "Offline" : "Online",
      subMode: "with-client",
      meetingLink: "",
      locationLink: "",
      clientVisibility: "With Client",
  ...
  const handleScheduleSubmit = async (e) => {
    ...
    try {
      ...
      const dataToSubmit = {
        ...
        positionId: cleanId(interviewForm.positionId),
        positionTitle: interviewForm.positionTitle,
        clientId: cleanId(interviewForm.clientId),
  ...
  const handleRescheduleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateInterview(rescheduleForm.id, {
        clientId: cleanId(rescheduleForm.clientId),
        clientName: rescheduleForm.clientName,
        positionId: cleanId(rescheduleForm.positionId),
        positionTitle: rescheduleForm.positionTitle,
        interviewDate: rescheduleForm.date,
        startTime: rescheduleForm.time,
        duration: parseInt(rescheduleForm.duration),
        meetingType: rescheduleForm.mode === "Online" ? "Video" : "In-Person",
        meetingLink: rescheduleForm.mode === "Online" ? rescheduleForm.meetingLink : rescheduleForm.locationLink,
        clientVisibility: rescheduleForm.subMode === "with-client" ? "With Client" : "Without Client",
      ...
      if (response.success) {
        ...
        toast.error(response.message || "Failed to reschedule interview");
      }
    } catch (error) {
  ...
  const handleReassignSubmit = async (e) => {
    ...
    try {
      ...
      const response = await updateInterview(reassignForm.id, {
        interviewerId: cleanId(reassignForm.interviewerId),
        interviewerName: reassignForm.interviewerName,
        interviewerRole: reassignForm.interviewerRole,
        interviewerEmail: reassignForm.interviewerEmail
      });

      if (response.success) {
        toast.success("Host re-assigned successfully");
        setIsReassignOpen(false);
        fetchInterviews();
      } else {
        toast.error(response.message || "Failed to re-assign host");
      }
    } catch (error) {
      console.error("Re-assign failed:", error);
      toast.error("Failed to re-assign host");
    } finally {
      setIsSaving(false);
    }
  };
            ...
            >
              ...
              <form onSubmit={handleScheduleSubmit} className="p-10 max-h-[75vh] overflow-y-auto dialog-scrollbar space-y-7 text-left">
                        ...
                        {(() => {
                          ...
                          return filtered.map((c) => (
                            <button key={c.id} type="button"
                              onClick={() => { setInterviewForm({ ...interviewForm, candidateId: c.id, candidateName: c.name, candidateEmail: c.email, positionId: c.positionId || "", positionTitle: c.positionTitle || "", clientId: c.clientId || "", clientName: c.clientName || "" }); setShowCandidateSuggestions(false); }}
                ...
                <div className="space-y-1.5 text-left" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="relative" ref={clientSearchDropdownRef}>
                    <div
                      onClick={() => {
                        setShowClientSearchDropdown(!showClientSearchDropdown);
                        setClientSearchTerm("");
                      }}
                    ...
                    >
                      <span>
                        {interviewForm.clientName || "Select Client"}
                      </span>
                      <ChevronRight size={14} className="text-[#1B4DA0] rotate-90 opacity-50" />
                    </div>
                    ...
                    {showClientSearchDropdown && (
                      <div className="absolute z-50 w-full mt-2 rounded-2xl shadow-2xl bg-white border border-[#F4F3EF] p-3 space-y-2 max-h-60 overflow-y-auto">
                        ...
                        <div className="space-y-1">
                          ...
                          {recruitmentClients
                            .filter(cl => {
                              const name = cl.companyName || cl.name || "";
                              ...
                              <button
                                ...
                                onClick={() => {
                                  setInterviewForm({
                                    ...interviewForm,
                                    clientId: cl.id,
                                    clientName: cl.companyName || cl.name,
                                    positionId: "",
                                    positionTitle: ""
                                  });
                                  setShowClientSearchDropdown(false);
                                ...
                                className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                  String(interviewForm.clientId) === String(cl.id)
                ...
                <div className="space-y-1.5 text-left" onClick={(e) => e.stopPropagation()}>
                  ...
                  <div className="relative" ref={positionSearchDropdownRef}>
                    <div
                      onClick={() => {
                        setShowPositionSearchDropdown(!showPositionSearchDropdown);
                        setPositionSearchTerm("");
                      }}
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] cursor-pointer flex justify-between items-center"
                    >
                      <span>
                        {interviewForm.positionTitle || "Select Position"}
                      </span>
                      <ChevronRight size={14} className="text-[#1B4DA0] rotate-90 opacity-50" />
                    </div>

                    {showPositionSearchDropdown && (
                      <div className="absolute z-50 w-full mt-2 rounded-2xl shadow-2xl bg-white border border-[#F4F3EF] p-3 space-y-2 max-h-60 overflow-y-auto">
                        <input
                          type="text"
                        ...
                        <div className="space-y-1">
                          ...
                          {(interviewForm.clientId 
                            ? recruitmentPositions.filter(p => String(p.clientId) === String(interviewForm.clientId))
                            : recruitmentPositions
                          )
                            .filter(p => p.title && p.title.toLowerCase().includes(positionSearchTerm.toLowerCase()))
                            .map(p => (
                              <button
                                key={p.id}
                                type="button"
                                ...
                                className={`w-full text-left px-4 py-2 rounded-xl text-xs font-bold transition-all ${
                                  String(interviewForm.positionId) === String(p.id)
                                    ? 'bg-blue-50 text-[#1B4DA0]'
                                    : 'text-[#1A1A2E] hover:bg-blue-50/30'
                                }`}
                              >
          ...
          {isRescheduleOpen && (
            <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-gradient-to-r from-white to-[#F8FAFF]">
                  <div>
                    <h3 className="text-2xl font-bold text-[#1A1A2E]" style={{ fontFamily: "'Syne', sans-serif" }}>Re-schedule Interview</h3>
                    <p className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest mt-1">Candidate: {rescheduleForm.candidateName}</p>
                  </div>
                ...
                <form onSubmit={handleRescheduleSubmit} className="p-10 max-h-[75vh] overflow-y-auto dialog-scrollbar space-y-7 text-left">
                  {/* Client Dropdown */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest pl-1">Client *</label>
                    <div className="relative">
                      <select
                        className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] appearance-none pr-10"
                        value={rescheduleForm.clientId || ""}
                        onChange={(e) => {
                          const selectedId = e.target.value;
                          const selectedClient = recruitmentClients.find(cl => String(cl.id) === String(selectedId));
                          setRescheduleForm({
                            ...rescheduleForm,
                            clientId: selectedId,
                            clientName: selectedClient ? (selectedClient.companyName || selectedClient.name) : ""
                          });
                        }}
                        required
                      >
                        <option value="">Select Client</option>
                        {recruitmentClients.map(cl => (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      <select
                        className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] appearance-none pr-10"
                        value={rescheduleForm.positionId || ""}
                        onChange={(e) => {
                          const selectedId = e.target.value;
                          const selectedPos = recruitmentPositions.find(p => String(p.id) === String(selectedId));
                          setRescheduleForm({
                            ...rescheduleForm,
                            positionId: selectedId,
                            positionTitle: selectedPos ? selectedPos.title : ""
                          });
                        }}
                        required
                      >
                        <option value="">Select Position</option>
                        {(rescheduleForm.clientId 
                          ? recruitmentPositions.filter(p => String(p.clientId) === String(rescheduleForm.clientId))
                          : recruitmentPositions
                        ).map(p => (
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input type="date"
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB]"
                      value={rescheduleForm.date ? rescheduleForm.date.split('T')[0] : ""}
                  ...
                  <div className="space-y-2">
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <button 
                        type="button"
                        onClick={() => setRescheduleForm({...rescheduleForm, mode: 'Online', subMode: 'with-client'})}
                        className={`p-4 rounded-2xl border-2 font-bold text-sm transition-all flex items-center justify-center gap-2 ${rescheduleForm.mode === 'Online' ? 'bg-[#EEF2FB] border-[#1B4DA0] text-[#1B4DA0] shadow-sm' : 'bg-[#FAFAFA] border-transparent text-gray-400 hover:border-gray-200'}`}
                      >
                        <div className={`w-2 h-2 rounded-full ${rescheduleForm.mode === 'Online' ? 'bg-[#1B4DA0]' : 'bg-gray-300'}`} />
                        Online
                      </button>
                      <button 
                        type="button"
                        onClick={() => setRescheduleForm({...rescheduleForm, mode: 'Offline', subMode: 'at-client'})}
                        className={`p-4 rounded-2xl border-2 font-bold text-sm transition-all flex items-center justify-center gap-2 ${rescheduleForm.mode === 'Offline' ? 'bg-[#EEF2FB] border-[#1B4DA0] text-[#1B4DA0] shadow-sm' : 'bg-[#FAFAFA] border-transparent text-gray-400 hover:border-gray-200'}`}
                      >
                        <div className={`w-2 h-2 rounded-full ${rescheduleForm.mode === 'Offline' ? 'bg-[#1B4DA0]' : 'bg-gray-300'}`} />
                        Offline
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest pl-1">
                      {rescheduleForm.mode === 'Online' ? 'Online Setup' : 'Offline Venue'}
                    </label>
                    <div className="relative">
                      <select
                        className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] appearance-none pr-10"
                        value={rescheduleForm.subMode}
                        onChange={(e) => {
                          const val = e.target.value;
                          setRescheduleForm({ 
                            ...rescheduleForm, 
                  ...
                  <AnimatePresence>
                    {rescheduleForm.mode === 'Online' && (
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          ...
                          />
                          <button type="button"
                            onClick={() => setRescheduleForm({ ...rescheduleForm, meetingLink: `https://meet.google.com/${Math.random().toString(36).substring(2, 5)}-${Math.random().toString(36).substring(2, 6)}-${Math.random().toString(36).substring(2, 5)}` })}
                            className="flex items-center gap-2 px-5 py-4 text-white text-sm font-bold rounded-2xl shadow-[0_10px_25px_rgba(27,77,160,0.3)] shadow-[#1B4DA0]/20"
                            style={{ background: 'linear-gradient(135deg, #1B4DA0, #3FA9F5)' }}>
                            Generate
                          </button>
                    ...
                    {rescheduleForm.mode === 'Offline' && rescheduleForm.subMode === 'at-client' && (
                      <motion.div 
                        initial={{ opacity: 0, height: 0 }} 
                        animate={{ opacity: 1, height: 'auto' }} 
                        exit={{ opacity: 0, height: 0 }}
                      ...
                      >
                        ...
                        <div className="flex gap-2">
                          <input
                            className="flex-1 bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] placeholder:text-[#9B9BAD]/50"
                            placeholder="https://maps.google.com/..."
                            value={rescheduleForm.locationLink}
                            onChange={(e) => setRescheduleForm({ ...rescheduleForm, locationLink: e.target.value })}
                          />
                        </div>
                      </motion.div>
                    )}
                  ...
                  <div className="space-y-1.5">
                    ...
                    <textarea rows={3}
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] placeholder:text-[#9B9BAD]/50 resize-none"
                      placeholder="Add rescheduling notes..."
          ...
          {isReassignOpen && (
            <div className="fixed inset-0 z-[1100] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="px-10 py-8 border-b border-[#F4F3EF] flex items-center justify-between bg-gradient-to-r from-white to-[#F8FAFF]">
                  <div>
                    <h3 className="text-2xl font-bold text-[#1A1A2E]" style={{ fontFamily: "'Syne', sans-serif" }}>Re-assign Interviewer</h3>
                    <p className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest mt-1">Candidate: {reassignForm.candidateName}</p>
                  </div>
                  <button onClick={() => setIsReassignOpen(false)}
                    className="w-12 h-12 rounded-2xl bg-[#F4F3EF] text-[#6B6B7E] hover:bg-red-50 hover:text-red-500 transition-all flex items-center justify-center shadow-sm">
                    <X size={20} />
                  </button>
                </div>

                <form onSubmit={handleReassignSubmit} className="p-10 max-h-[75vh] overflow-y-auto dialog-scrollbar space-y-7 text-left">
                  {/* Interviewer Name with suggestions */}
                  <div className="space-y-1.5 relative">
                    <label className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest pl-1">Interviewer Name *</label>
                    <input
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] focus:ring-2 focus:ring-[#1B4DA0]/10 placeholder:text-[#9B9BAD]/50"
                      placeholder="Assign member"
                      value={reassignForm.interviewerName}
                      onChange={(e) => { 
                        setReassignForm({ ...reassignForm, interviewerName: e.target.value }); 
                    ...
                    <AnimatePresence>
                      {showInterviewerSuggestions && (
                        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -10 }}
                          className="absolute z-[130] w-full mt-2 bg-white border border-[#F4F3EF] rounded-xl shadow-xl max-h-60 overflow-y-auto">
                          {(() => {
                            const filtered = availableInterviewers.filter(s => (s.name || "").toLowerCase().includes((reassignForm.interviewerName || "").toLowerCase().trim()));
                            if (filtered.length === 0) return <div className="px-4 py-6 text-center text-[#9B9BAD] text-[10px] font-bold uppercase tracking-widest">No members found</div>;
                            return filtered.map((s) => (
                              <button key={s.id || s.name} type="button"
                                onClick={() => { 
                  ...
                  <div className="space-y-1.5">
                    ...
                    <input
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] placeholder:text-[#9B9BAD]/50"
                      placeholder="e.g., Tech Lead"
                      value={reassignForm.interviewerRole}
                      onChange={(e) => setReassignForm({ ...reassignForm, interviewerRole: e.target.value })}
                    />
                  </div>

                  {/* Interviewer Email */}
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-black text-[#9B9BAD] uppercase tracking-widest pl-1">Interviewer Email</label>
                    <input
                      type="email"
                      className="w-full bg-[#F4F3EF] border-0 rounded-2xl px-6 py-4 text-sm font-bold text-[#1A1A2E] outline-none transition-all focus:bg-[#EEF2FB] placeholder:text-[#9B9BAD]/50"
                      placeholder="interviewer@company.com"
                      value={reassignForm.interviewerEmail}
                      onChange={(e) => setReassignForm({ ...reassignForm, interviewerEmail: e.target.value })}
                    />
                  </div>

                  {/* Footer Buttons */}
                  <div className="pt-4 flex gap-4">
                    <button type="button" onClick={() => setIsReassignOpen(false)}
                      className="flex-1 py-5 rounded-3xl border-2 border-[#F4F3EF] text-sm font-bold text-[#6B6B7E] hover:bg-[#F4F3EF] transition-all">
                      Cancel
                    </button>
                    <button 
                      ...
                      className="flex-[2] bg-[#1A1A2E] text-white py-4 rounded-2xl text-sm font-bold shadow-xl hover:bg-black transition-all flex items-center justify-center gap-2 disabled:opacity-50"
                    >
                      {isSaving ? "Saving..." : "Re-assign Host"}
                    </button>
                  </div>
                </form>
              </motion.div>
          ...
          {selectedRowIds.length > 0 && (
            ...
            >
              ...
              <div className="flex items-center gap-4">
                <button
                  onClick={() => {
                    const first = interviews.find(i => String(i.id) === String(selectedRowIds[0]));
                    if (first) {
                      setRescheduleForm({
                        id: first.id,
                        candidateName: first.candidateName,
                        clientId: first.clientId || first.raw?.clientId || "",
                        clientName: first.clientName || first.raw?.client?.companyName || first.raw?.client?.name || "",
                        positionId: first.positionId || first.raw?.positionId || "",
                        positionTitle: first.positionTitle || first.raw?.position?.title || "",
                        date: first.date || "",
                        time: first.time || "",
                        duration: String(first.duration || "45"),
                        mode: first.meetingType === "In-Person" ? "Offline" : "Online",
                        subMode: first.clientVisibility === "With Client" ? "with-client" : "without-client",
                        meetingLink: first.meetingType === "Video" ? (first.meetingLink || "") : "",
                        locationLink: first.meetingType === "In-Person" ? (first.meetingLink || "") : "",
                        notes: first.notes || ""
                      });
                  ...
                  onClick={() => {
                    ...
                    if (first) {
                      setReassignForm({
                        ...
                        interviewerName: first.interviewerName || first.interviewer || "",
                        interviewerRole: first.interviewerRole || "",
